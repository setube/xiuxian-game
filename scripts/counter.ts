 ...
 *
 * ## 三刀验收（`bash scripts/cut.sh`，2026-09-12 跑的）
 *
 *     A 刀　`meetsAll` 恒真　　→ 红 8 项
 *     B 刀　`applyEffects` 空转→ 红 6 项
 *     C 刀　`meetsAll` 恒假　　→ 红 9 项（全库 107 支：绿 34 红 73）
 *
 ...
 *
 * C 刀底下印出来的话，**正是前两版写坏时的真实样子**：
 *
 *     ✗ 4 个摆局只落到 1 个不同的节点——分流条件有恒假的：
 *         「grew-old」同时收下了：掌柜没了、四十四岁掌柜还在、
 *                                 二十七岁家里有地、三十五岁
 *
