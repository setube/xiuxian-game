 ...
 * 采到多少步是在高墙里头过的日子。
 *
 * 没有这个数，底下那一条会在「一步也没采到」时打勾——改判维度那次真发生过：
 * 取错了字段（`living` 是对象不是字符串），「一步也没采到」而第二问照样绿。
 *
 * ## ⚠️ 这个数只判得了「零和非零」，判不了「够不够」
 *
 * 它是 `600 世 × 是宗室的比例 × 每世步数` 乘出来的——**没有作者**。
 * 改宗室出身的概率、改任何一章的窗口、往库里加一章新内容，它都会动，
 * 而**没有任何一层的作者认为自己在决定它**。
 *
 * 2026-09-09 有人实测过这件事：17 加了一章节令，`kindred` 报的
 * 「侄儿成人」从 29 世掉到 24 世——**那一支的作者什么也没做，数却动了**。
 * 给这种数定阈值，等于让每个上游改动都可能红在下游头上。
 *
 * 所以这一条只问「是不是零」：零 = 这一支什么也没量到，非零 = 量到了。
 * **不问「963 步够不够」**——那个问题没有答案，因为没人在决定它该是多少。
 *
