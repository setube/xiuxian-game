 ...
 *
 * ⚠️ **这份名单跟库里那几条 `living notIn` 长得像，但不是同一份，别去「对齐」它。**
 *
 * 库里现在写的是 `['palace', 'manor', 'up-there']`（`day.ts:71` 那几处），
 * 比这儿多一档——**而那一档是有意不在这儿的**。两份名单问的不是同一件事：
 *
 *     库里那份　　哪些日子不该开「出去做工」那个选项
 *     这儿这份　　哪些日子算「高墙里头」，要拿来跟寒门比选项集合
 *
 * `up-there` 是半山腰（`content/living.ts` 的 `UP_THERE`：「挑水、烧灶、听人使唤」）。
 * 他不出去做工，所以进库里那份；可他**干的就是活**，拿他跟王府世子并成一类去比
 * 「高门读不读得到干活选项」，比出来的是噪声——这一支要抓的是
 * 「王府世子读到了『帮家里干活』」，不是「挑水的人读到了挑水」。
 *
 * 2026-09-11 我自己按「两份名单不一致」查过一轮，`b62ef33`（加 `up-there` 那笔）
 * 正是我提交的，一度当成自己漏改的。**不一致是对的，缺的只是这段话。**
 */
