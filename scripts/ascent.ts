 ...
 * 窗口没跟着挪。这一笔：三册窗口挪到成年段末（`GROWN_UP`）；药庐加「再去一趟」那一节
 * （他要的正是肯天天守着的人）；修士不按凡人公式老死（陶仲八十三岁入册，几年内会「老病」）。
 *
 *   一、摆好的局：十四岁、念头在、头一回被晾（旗标没落）→「再去一趟」开着；心志够了再去，
 *       他说「你若真想学，明日再来」，落「搭话」，往后接的是使唤那一卷；自己说不去了的回不来。
 *   二、窗口从段表取：找人、照面、师承三册的事件 `to` 都等于 `GROWN_UP`，目录跟着；
 *       「再去一趟」带 `chance`（宽窗口可反复的事没有它会挤光日常）。
 *   三、修士不老死：陶仲入册带 `realm`，推四十年还在；对照：一个八十五岁的凡人推四十年会没。
 *   四、随机人生（分片，两种走法）：漏斗表只报数；有心人那一批掷到甲二、丙四、乙五、乙六
 *       各至少一世为止（上限十倍）；链的单调性（到了乙六必到过乙五……）；不许太好走
 *       （有心人第一次修炼 ≤ 20%，随机 ≤ 10%——`seeking.ts` 那条「两成以上走到门前就太好走了」）。
 */
import './lib/seeded'
...
import { GROWN_UP } from '../src/engine/stages'
...
// ============================================================
{
  ...
  if (!s) wrong.push('掷不出局')
    ...
    if (!again) wrong.push('库里没有「再去一趟」这条事件')
      ...
      if (again.chance === undefined || again.chance > 0.3)
        wrong.push(
          `「再去一趟」是宽窗口可反复的事，得有 chance 且不大：${again.chance ?? '（无）'}`,
        )
...
// ============================================================
{
  ...
  for (const id of chained) {
    ...
    else if (event.window.to !== GROWN_UP)
      wrong.push(
        `${id} 的窗口到 ${event.window.to} 为止，该到成年段末 ${GROWN_UP}——成年那一段人生早有了`,
      )
  }
  for (const id of ['seeking', 'meeting', 'tutelage']) {
    const chapter = CHAPTERS.find((one) => one.id === id)
    if (!chapter) wrong.push(`目录里没有 ${id}`)
    else if (chapter.age[1] !== GROWN_UP)
      wrong.push(`目录里 ${id} 写的是到 ${chapter.age[1]}，事件到 ${GROWN_UP}`)
  }
  // 一个二十四岁的人，念头在，这几卷对他开着；三十岁就关了
  ...
  if (wrong.length > 0) {
    ...
    console.log(
      `  ✓ 二、找人、照面、师承九条事件都开到成年段末（${GROWN_UP} 岁）；目录跟着；「再去一趟」带 chance。`,
    )
}
...
// ============================================================
{
  ...
  const MUST = ['errand', 'temple', 'attempt', 'felt']
   ...
   * 一世也没到过的环印出来，不判红。
   *
   * 药庐那条链（带一段 → 那五句 → 自己坐）在真世里几乎走不到：陶仲量的是心志和手脚，
   * 「带一段」要 74、「教一点」要 82，而念头多半在十几岁上起、二十几岁就退了，
   * 那时候心志中位 51–58。这是设计（他量的就是肯不肯守着），不是断路——
   * 摆局里心志 95 的人四趟走到底（第一条）。**但它该被看见**：哪天有人改了念头的退法
   * 或陶仲的门槛，这一行会先变。
   */
  ...
  if (wrong.length > 0) {
    ...
    console.log(
      `\n  ✓ 四、有心人 ${keen.length} 世里，使唤、观里、第一次修炼、觉出了什么各至少一世到过；链单调；第一次修炼有心人 ${Math.round(keenAttempt * 100)}%、随机 ${Math.round(randomAttempt * 100)}%，不算好走。`,
    )
}
