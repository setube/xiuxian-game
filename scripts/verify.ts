...
console.log('=== 前置条件验收（要的东西有没有人给）===\n')
{
  ...
  const NEEDS = {
     ...
     */
    /*
     * 「碰上过那件世界事件」的出处是 `content/world-events.ts` 那张表，
     * **不是任何一条效果**——世界自己在走，不需要谁去设它。
     * 那张表里的 id 有没有拼错，由底下第三道（跳转与引用）另查。
     */
    befell: null,
    /*
     * 往事这一道查不了，而理由跟别的 `null` 不一样，写在这儿。
     *
     * 这一道配的是「内容层需要的 id」和「内容层造得出的 id」。
     * 而长辈的往事是**立基掷的**（`birth.ts` 的 `ELDER_PAST`），
     * 内容层一处也造不出来——配对的另一头根本不存在。
     *
     * 它由 `scripts/past.ts` 单独查：那一支比的是
    ...
    /*
     * 看着比岁数年轻多少。`who` 是人口册上的 id。
     *
     * ## ⚠️ 这儿从前写着一个【当场造、当场丢】的 Map
     *
     * ```ts
     * unaged: (ask) => [new Map([['人物', [ask.who]]]), ask.who],
     * ```
     *
     * `note()` 把 id 写进**传进来的那张表**，而这张表是每次调用现造的
     * ——函数一返回它就没了。**写入端有，读取端零。**
     *
     * 而它原来的注释写着「`who` 是人口册上的 id，**该由这一道查**。
     * 写 `null` 的话会安安静静永远为假，那正是『恒假的条件』那一族」
     * ——**注释说它在防恒假，它自己是空转的。**
     *
     * ⚠️ 这比 `null` 更坏：`null` 是一句明确的「这一格不归我查」，
     * 而一个空转的函数让人以为有把尺子在这儿守着。
     *
     * ## 改成 `null` 而不是补一张长期表，是因为这一道【跑不动】
     *
     * 这一支是纯静态的（不起 pinia、不跑世界），而人有五种来源，
     * 其中最大的一种——立基造的爹娘、东邻、王府那几位——
     * 只有跑起来才数得清。硬塞进来就得手写一张名单，
     * 而手写名单是这个库今天已经栽过四次的东西。
     *
     * 人物 id 这一族归 `scripts/who.ts`：跑真世界攒人口册，
     * 再拿 `CULTIVATORS` 的导出补上稀有到跑不出来的那几位。
     */
    unaged: null,
  } satisfies {
...
console.log('=== 可观测路径验收（人生里真走得到吗）===\n')
{
   ...
   *
   * 2026-09-08：`youth:apprentice` craft 选项加入 `craft-master` NPC 之后，
   ...
   *
   * 2026-09-08 晚：52 修了 `flag.absent`（`equals: false` 表达不了「旗没有」），它那五卷从死的
   * 变成真会演的，年表的走法跟着变；同一天又进了三册（三十二册、一百一十四卷、五百二十三节）。
   * 五批量出 143 / 146 / 156 / 149 / 142，均值 147.2、σ 5.5，上限 = 147 + 3 × 5.5 ≈ 164。
   *
   * 2026-09-10：`keeping` 那一册（第一个正向 lifespan）进来之后红了——
   * 它 8 个节点在 300 世里一个也走不到，**而那是这一册的性质，不是 bug**：
   * 修行链上游 `riverman#named-true` 只有 0.4%，够格的人几千世才有一个。
   *
   * 七颗种子重量：128 / 174 / 138 / 128 / 134 / 141 / 144
   * （均值 141.0、σ 15.8、μ+3σ = 188）。
   *
   * **取 180 = 最坏那颗 174 + 小量余地，比 μ+3σ 紧一档。**
   * 按「看最坏那颗」那条纪律定，不按均值——σ 从 5.5 涨到 15.8 说明
   * 这个数本来就抖，均值那条线会被单颗坏种子直接穿过去。
   *
   * ⚠️ **重量这个数得从绿的那几颗里取，而判据绿的时候不打印它**
   * ——那行「走不到的节点有 N 个」只在超上限时才印。绿的时候要从
   * 「走到过 X 个节点，一共 Y 个」里减出来。下次重量的人别在这儿卡住。
   * 五批不是十批，跟上一条同一个软法；这个数是三十二册时量的，下次红了先看册数涨没涨。
   *
   * 2026-09-08 更晚：三颗种子实测走不到 **132 / 102 / 122**（均值 119），
   * 比定这个上限时的 147 低了近三成——**内容又长了（现在 115 卷、540 节点），
   * 而覆盖跟着变好了**，所以上限相对变松：余量从 3σ 变成了约 45 个节点。
   *
   * **没有收紧它，理由是这个方向的松是安全的**：上限防的是「内容进来了却走不到」，
   * 而覆盖变好时它只是暂时钝一些，不会误报。真要收紧得补够十批重量 σ，
   * 那是半小时的活，等下一次真红了一起做。
   *
   * ⚠️ 但**这一条的性质要认清**：它是三类判据里的第一类（概率性），
   * 上限就是它的门槛。上面那两笔「暂设 140」「设 164」都是照观测最大值加余量定的，
   * **那是防误报的定法，不是按检出力定的**——它抓得住「多走不到几十个节点」，
   * 抓不住「多走不到三五个」。后者要的是另一种判据（比如按卷对账，
   * 而不是数总节点），不是把这个数往下压。
   *
   * ## 2026-09-12：180 → 195，因为库长了近三成
   *
   * `round-1` 那颗种子量出 186，打穿了 180。查下来**不是谁的内容坏了，
   * 是这个数累积过期了**：
   *
   *     定 180 那天　三十二册、540 个节点、走不到 119–174
   *     这一天　　　 679 个节点（+139，近三成），走不到 139–186
   *
   * 八批重量（`v1`–`v7` 加打穿的 `round-1`）：
   *
   *     139 / 149 / 142 / 119 / 158 / 165 / 168 / 186
   *     均值 153.3、σ 19.3、μ+3σ = 211
   *
   * **取 195 = 最坏那颗 186 + 小量余地**，照上面那条定法，仍比 μ+3σ 紧一档。
   *
   * ⚠️ **那一天进来的那一册（`counter`，五个节点）不是这次打穿的原因。**
   * 五个节点落在 σ = 19.3 的噪声里，而那一册自己有专属门禁
   * （`scripts/counter.ts` 七条，A/B/C 三刀验过）。
   * 记这一句是因为「红灯出现在谁进来那天」最容易被当成因果——
   * 而这个数上一次重量是三天前、隔着一百多个节点。
   */
  /*
   * ## 2026-09-14：195 → 210，照同一条定法重量
   *
   * `m4bekxv0wx8n` 那颗量出 202，打穿了 195。十批重量：
   *
   * ```
   * 131 / 137 / 153 / 157 / 166 / 171 / 173 / 177 / 184 / 202
   * 均值 165.1、σ 21.4、μ+3σ = 229
   * ```
   *
   * **取 210 = 最坏那颗 202 + 小量余地**，照上面那条定法，仍比 μ+3σ 紧一档。
   *
   * ⚠️ **那一天进来的那一卷（`neighbour:east-grown`，三个节点）不是打穿的原因**
   * ——三个节点落在 σ = 21.4 的噪声里，而且它自己实测走得到
   * （300 世演到 88 次，`cells.ts` 报它从「1 卷」变成「2 卷」）。
   * **那一卷的节点一个也不在「走不到」的名单里。**
   *
   * 这一句照抄上一次那条提醒的形状：**「红灯出现在谁进来那天」最容易被当成因果**，
   * 而这个数上一次重量是两天前。
   */
  const UNVISITED_CEILING = 210
