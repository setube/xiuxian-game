...
const KNOWN_BARE = new Set([
  ...
  'east-wife',
  /*
   * 玩家的儿子。跟上面两位同一个理由，而**挡法是【入场条件】**——
   * 比 `seen` 和分流都靠前：
   *
   *     son-grown 的 requires: [{ family: { id: 'son', alive: true, age: { atLeast: 16 } } }]
   *
   * 一手验过：空库（没有儿子）时那一条为 `false`，
   * **这一卷整个演不到**，`{call:son}` 那句正文印不出来。
   *
   * ⚠️ 加进这份名单不等于「这一处不用管」——是**这一处已经管住了**。
   * 往后要是有人在【没有入场条件保护】的地方写 `{call:son}`
   *（比如某一卷的 `seen` 里顺手点他的名），这份名单帮不上忙，
   * 那时候要么给他兜底称呼，要么那句话改成不点名。
   */
  'son',
  /*
 ...
 *
 * ## ⚠️ 它认死了 `{call:spouse}` 这一种写法，而那是靠「恰好」成立的
 *
 * 上头那个循环只认 `{call:spouse}`。全库配偶确实只能这么写——
 * 因为**配偶不在角色记号名单里**：
 *
 *     interpolate.ts:235   ROLE_IDS = ['elder', 'dam', 'child', 'playmate']
 *     正文实测              {call:spouse} 15 处、{call:playmate} 1 处
 *
 * **而这不是设计出来的，是碰巧。** 角色记号由关系网现算（`{elder}` 落到谁
 * 取决于生父/抚养/生母谁还在），**静态解不开**——哪天有人给配偶加一个记号，
 * 这一条当场变成漏报，**而它会继续绿着**。
 *
 * 所以往 `ROLE_IDS` 里加东西的人：**回来看一眼这一条**。
 * 加的那个记号要是可能落到配偶身上，这里的 `{call:spouse}` 得跟着扩。
 *
