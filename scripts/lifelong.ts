 ...
 *
 * 定这个数那天量下来：启蒙停过 12 世，少年停过 8 世。**都是个位数**，
 ...
 *
 * ## 2026-09-13 照上面那句重量了一次，两个数都漂了，而且漂得不对称
 *
 * ```
 *              定 6000 那天   2026-09-13（SEED=re-79，同样 6000 世）
 * 启蒙              12 世            185 世      ← 涨了十五倍
 * 少年               8 世             12 世      ← 只涨了半档
 * ```
 *
 * **为什么不对称**：这两格的稀有不同源，而**不是段宽能解释的**。
 *
 * 按段宽归一化（同一次 6000 世的实测）：
 *
 * ```
 *          年数    停过世数    停的回数    回/年
 *   幼年     6        5987      15341    2556.8
 *   启蒙     6         185        374      62.3
 *   少年     4          12         12       3.0   ← 比启蒙低 21 倍
 *   成年    13        5999      32410    2493.1
 *   壮年    20        6000      52812    2640.6
 * ```
 *
 * ⚠️ 我先写下的说法是「少年不是概率稀有，是结构窄（只有四年）」——**那话错了，
 * 归一化当场推翻**：段宽只差 1.5 倍（六年对四年），而密度差 21 倍。
 * 窄只解释得了其中一小部分，剩下十几倍是别的东西。**没查清之前不要写下机制**。
 *
 * 那十几倍是什么，这一支答不了（它只数停过几回，不数为什么没停）。
 * 要查得去问「12–15 岁那四年，候选池里常年挂着哪几件事」——
 * 而那是内容层的账，不是这个数的账。
 *
 * ## 这个数现在该不该动：不动，而两个方向的理由不一样
 *
 *     启蒙 185 世   λ≈185，e⁻¹⁸⁵ 小到没意义。**六千对它是十几倍的过度保险**
 *     少年  12 世   λ≈12，e⁻¹² ≈ 六百万分之一。够，但**它是全套最贵一支的承重墙**
 *
 * 砍到三千：启蒙仍然稳，少年 λ≈6，假红率回到四百分之一——**那是会真的撞上的量级**
 * （这一支 134.6s，全套最贵，占 14.3 分钟里的 15%；省一半耗时换一个每四百轮
 * 报一次假红的判据，不划算——CLAUDE.md「门禁慢别砍世数」那条说的正是这个）。
 *
 * ⚠️ **下次重量时先看少年那一格，不要看启蒙。** 启蒙已经宽裕到不构成约束了，
 * 而那两个数从前是并排写的，看着像同一件事的两个样本——**它们不是**。
 * 真正卡着 `RUNS` 的只有少年那一格。
 */
const RUNS = 6000
 ...
 * 一、一生跑得完，而且是在终点停下的。
 *
 * ## ⚠️ 这一条红的时候，第五条会跟着红——那是同一世，不是两个 bug
 *
 * 2026-09-12 一手查清的：撞上回合上限那一世，会**同时**点亮三条红：
 *
 *     这一条　　　N 世按满了 600 下还没走完
 *     这一条　　　N 世没走到落幕
 *     第五条　　　N / 6000 世没活到自己最终的天年
 *
 * **而 N 恒等**。原因在采样点：`tasks/lifelong-lives.ts:173` 的
 * `spanAtDeath` / `died` 是在 while 循环**退出之后**采的，而循环有三个出口
 * （走到落幕、无可选项、撞上限）。后两种是**人还没死就退出了**，
 * 那时 `age < span` 天然成立——第五条量到的不是夭折，是那一世没跑完。
 *
 * 所以红了只需要查一件事：**为什么那一世停不下来**。
 * 别顺着第五条去翻 `lifespan.ts` 找夭折机制——那里没有，
 * 三条红是一个成因的三种症状（同「一批效果会各自散开」的反面）。
 *
 * 实测频率约 1/6000，换一颗种子多半就不出现——**而那不是噪声，
 * 是一个真实的低频循环**：恒定出现一世、恒定三条同亮，噪声不会这么整齐。
 */
 ...
 * ## 这一问红了意味着什么
 *
 * ⚠️ **先看第一条红了没有。** 2026-09-12 查清：撞上回合上限那一世
 * 会让这一条跟着红，而那不是夭折——`spanAtDeath` 是在 while 循环退出后采的，
 * 那一世人还没死就退出了，`age < span` 天然成立。
 * **第一条和这一条同时红、且世数相等，就是同一世的一个成因两种症状。**
 *
 * 只有第一条绿、这一条单独红，才是真的有人给这个世界加了夭折——
 * **那也不一定是错的**（`lifespan.ts` 那段
...
function noOneDiesEarly(tally: Tally): string[] {
  ...
  return [
    `${early.length} / ${tally.died.length} 世没活到自己最终的天年（最多差 ${worst} 岁）——` +
      '⚠️ 先看第一条：撞上回合上限的世会让这一条跟着红（采样点在循环退出后，' +
      '那一世人还没死），两条世数相等就是同一世。' +
      '只有第一条绿、这一条单独红，才是真有人加了夭折' +
      '（`engine/lifespan.ts:32` 明写库里没有，那是有意的）。',
  ]
}
