 ...
 * 够第二、三条踩实。第五条会把实际采到多少印出来。
 *
 * ## 「采不够会红」——这句话从前写在这儿，是假的
 *
 * 底下六条哨兵判的都是 `saw* === 0`：**一次也没采到才红，采得不够不红**。
 * 两者差得远，而写成前者会让人以为这个数有人守着。
 *
 * 这一支跟 `newyear` 那笔账（世数只为一条判据算过，另一条一直蹭余量）
 * 不同源——**这里没有一条判据按比率判**，九条全是零容忍
 * （`offences.length > 0` 就红），撞上一次就认得出，样本量只决定
 * 「多久撞上」。所以调世数不会抽掉谁的底。
 *
 * 欠的是另一层：要抓「只在某种亲缘形状里才犯」的毛病（`edgeKind`
 * 从前只看 bond、把姐姐画成娘那种），≥1 不够。按九成把握至少撞一次
 * （`n ≥ ln(0.1)/ln(1-p)`），违例率两成要 11 个样本、一成要 22 个。
 * 眼下靠印出来的数给人看，没有一行代码守着。真被咬一次就把哨兵的门槛
 * 从「非零」抬上去——详见 `scripts/lotpath.ts` 里同一段。
 */
const RUNS = 300
