 ...
 * > ——**写清楚，比让报表好看有用。**
 *
 * ## ⚠️ 这次收窄的性质：不是覆盖率问题，是【辨识力】问题
 *
 * > 第一版的主要缺陷**不是覆盖率不足，而是判定谓词的辨识力不足**。
 * > 旧谓词只证明「**存在相容解释**」，不能证明「**目标解释唯一成立**」。
 *
 * ```
 * 旧   这个 id 造过的姓里有没有它      存在相容解释 → 永远找得到
 * 新   这个 id 是不是【只有】一个姓    目标解释唯一成立
 * ```
 *
 * 收紧之后覆盖率从 100% 掉到 1/3，**而判不了的两项恰与历史错绑完全重合**
 * ——那说明新的打断验开始量「历史故障真正需要的语义分辨能力」，
 * 而不是单纯增加校验数量。
 *
 * ⚠️ **而旧谓词那种状态最危险**：
 *
 * > 任何错误都能找到一个「至少有一点沾边」的证据，于是永远不会红。
 * > 这比普通的漏检更坏，因为它制造一种**非常稳定的假证明感**。
 *
 * 所以往任何一支判据里加「对照」时，先问一句：
 * **这条对照证明的是「存在一种说得通的解释」，还是「我要的那个解释唯一成立」？**
 */
