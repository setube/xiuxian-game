 ...
 *
 * 这跟补掷循环里「命中率现算」是同一条道理，只是提到了最前面。
 * **写死的率一个也不留**，谁改了内容都不用回来改这儿。
 */
/**
 * 探路批掷多少世。
 *
 ...
 *
 * 所以取 8。**这个 8 是防零余量，不是检出力数**——两者按完全不同的式子算：
 *
 *     ①概率性判据    n ≥ ln(0.1)/ln(1-p)      要「抓得住多低的违例率」
 *     ②恒等式判据    P(入样为零) = (1-r)^N     只要「别掉到零」
 *
 * 按后者：率 1.2%、总世数 867，入样为零的概率是 2.8e-5，8 绰绰有余。
 *
 * ⚠️ **别拿 `newyear.ts` 里那张检出力表来读这个 8。** 8 在那张表上对应
 * 25% 的违例率，看起来低得该抬——**而那张表对这一条根本不适用**：
 * ②坏了的话每个入样的世都会红，一个样本就够。按①去抬它，
 * 就是我刚走过的那条路：三倍耗时换零收益。
 *
 * 同理，`TWICE_WANTED = 8` 和 `newyear.ts` 的 `WANTED_FOR_SEASON = 22`
 * 并排站着，**不是「这条比那条宽松」——它们量的不是同一个东西**。
 */
...
// 八、老屋的营生
{
  ...
  if (wrong.length > 0) {
    ...
    /*
     * 那个覆盖数**每一次都是 0**，三颗种子实测（624 / 1170 / 851 世）无一例外。
     *
     * 不是稀有，是**结构上到不了**：`repaid` 要走完「荒年撞上分家 → 借了粮 →
     * 又到还粮那一年」整条链，而头一步（借粮）实测率就只有 0.2%
     * （见 `BORROWS_WANTED` 那段注释），后面还要再撞两次。
     *
     * 判据本身在摆好的局上量，绿的是真的。但**印在 ✓ 后面的一个 0
     * 看着像出事了**——xiuxian-game-17 2026-09-09 就是这么问过来的，
     * 它怀疑是我 `5ce8c45` 改取样改坏的。查完是既有的、预期的。
     *
     * 所以这一行自己把话说全，别让下一个读到它的人再查一遍。
     * **一个恒为零的报数，比一个偶尔为零的更该解释**——偶尔为零的人会
     * 等下一轮，恒为零的人会去翻代码。
     */
    console.log(
      `  ✓ 八、还粮那一卷读了老屋的营生：种地的还粮、看收成；开铺子的折银子、看粮价。` +
        (randomly === 0
          ? '（随机人生里一世也没走到，预期如此：借粮 0.2%，还得再撞上还粮那一年——判据在摆好的局上量）'
          : `（随机人生里也走到 ${randomly} 世）`),
    )
  }
}
