 ...
 *
 * ## 谁定的：我按实测定的（79，2026-09-08），**用户没拍过**
 *
 * 这一列是 2026-09-09 补的，起因是 `riverman.ts` 那个 `fortune >= 50`
 * ——**那个数是用户 09-08 亲自拍的**（55 → 50，他当时看到的对照是 4% → 15%），
 * 而两天后被别人加的内容稀释到 11%，**没有任何地方记着「这是他定的」**。
 * 于是它看起来跟一个随手写的数一模一样，谁都可以改。
 *
 * 所以这一列要答的是一个操作问题：**改它之前要不要问一声。**
 *
 *     用户拍板　　改之前问　　　　　　　`riverman.ts` 那个 50
 *     从系统读的　跟着源头改，没有取舍　`doors.ts` 的 `DEATH_STARTS_AT`
 *     我按假设写　直接改，说出理由即可　这一处、`doors.ts` 的 `LOCKED_ODDS`
 */
const TOLERANCE = 0.2
