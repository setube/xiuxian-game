 ...
 * ## 这条注释从前是空话
 *
 * 2026-09-12 量出来：分片那一格挑的是 `seen`（40.2s，27 支分片支里**第三慢**），
 * 而这一支要跑它三回，于是自己 96.0s、全套第二慢——**注释写着「挑快的」，
 * 代码挑的是最慢的那几支之一**。一条声明了意图却没人核对的注释，
 * 跟没有注释的区别是：它会让下一个人以为已经挑过了。
 *
 * 换成 `mourning`（2.0s），二十倍。挑它的依据不是「看着快」，是实测判别力——
 * 这一支要的就两件事，候选**五支全测过**（抹掉 pid/时刻/run 再比，
 * 不抹的话种子行每跑必变，五支会全报「同种子不同」，那是在量时钟不是量种子）：
 *
 *     候选        同种子两回相同   异种子不同
 *     mourning    ✓                ✓          ← 选它，2.0s
 *     ruin        ✓                ✓             3.3s
 ...
 *     deeds       ✓                ✓             1.9s
 *
 * 五支都合格，所以这一格**不是只有一个选择**——`mourning` 坏了随时可换，
 * 换之前把上面两问重测一遍就行。
 *
 * ⚠️ 换支之后要打断验证：把这里换成一支不装种子的，这一支必须红。
 * 不红说明换完之后这把尺子量不动了（同「阈值一变判据就空跑」那一族）。
 */
const PAIRS: readonly { name: string; shards: string }[] = [
  { name: 'kept', shards: '1' },
  { name: 'mourning', shards: '3' },
]
