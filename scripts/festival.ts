...
let pushed = 0
let pushes = 0
/** 撞上回合上限、没走完一辈子的世数。**报数不判红**——它是读覆盖数时的背景 */
let truncated = 0
/** 有几世从 `drain()` 读到过正文。**对照行：答案已知，必须非零**（见尺子自检那一段） */
let sawAnyText = 0
let worlds = 0
...
for (let i = 0; i < RUNS; i += 1) {
  ...
  /*
   * 这一世是自然走完的，还是撞上回合上限被截断的。
   *
   * ## 为什么要数它：节令让「一回合推多久」变短了
   *
   *     节令一回合　　平均推 4.5 个月
   *     日常一回合　　推两三年（`routine:adult` 两年、`routine:prime` 三年）
   *
   * **插入节令回合，同样的年数就要更多回合走完**——而回合有上限（240）。
   * 撞上限的人生在天年之前被截断，**晚年才发生的事因此少了**。
   *
   * 这件事要紧，因为它跟「节令挤占了别的事件」在报表上长得一模一样，
   * 而含义完全相反：
   *
   *     挤占候选　　节令抢了别人的回合　　　内容设计问题，该调 chance
   *     被截断　　　240 回合走不完一辈子　  **量具问题，跟内容无关**
   *
   * 我在 `1b24935` 里报「中秋让要熬年数的链条推后约四分之一」时只写了前一种
   * ——数是实的，解释缺了一半。**这一格是为了让下一个人不必重问一遍。**
   *
   * 报数不判红：撞上限本身不是坏事（长寿的人生本来就回合多），
   * 它是一条**读别的门禁的覆盖数时要先看的背景**。
   */
  if (!narrative.ended) truncated += 1
}
...
for (const festival of FESTIVALS) {
   ...
   *
   * ## 年节报出的那个 5%，出处在这儿——**别去修年节**
   *
   * 报表里 `kindred:newyear` 演到过约 5%，看着低得像内容坏了。它不是：
   *
   *     分家实测 6%（79 量的，2026-09-09，主干 db65fab 前后，600 世，
   *                  两条路径交叉：`meetsAll` 逐层、`houses['old-home']` 存在）
   *     年节 5%　　  这一支同期实测（300 世，另一个方向、另一套采法）
   *
   * **两个数是同一件事的两次独立观测**：年节的天花板就是分家率，
   * 侄儿满三岁那一层几乎不再削减它。`kindred` + `away` + `nephew`
   * 那两千多行内容全挂在这 6% 上。
   *
   * 所以这个 5% 不是「年节写得不好」，**是它站在一条 6% 的链上**。
   * 要抬只能抬分家率，那是家户那一片的设计决策，不是节令这一片的事。
   *
   * ⚠️ 这两个数会漂（分家率一改它俩一起动），**它们是出处不是门槛**。
   */
...
/*
 * 撞上回合上限的世数。**报数不判红**，但它是读别的门禁覆盖数时的背景。
 *
 * 节令一回合平均推四五个月，日常一回合推两三年——**插入节令回合，
 * 同样的年数要更多回合走完**。撞上限的人生在天年之前被截断，
 * 晚年才发生的事（侄儿成人、到第三代）因此少了，
 * **而那跟「节令挤占了别的事件」在报表上长得一模一样**。
 */
console.log(
  `        撞上 240 回合上限、没走完一辈子的：${truncated} / ${worlds} 世` +
    `（${((truncated / Math.max(worlds, 1)) * 100).toFixed(1)}%）` +
    `　（报数不判红——它是读别处覆盖数时的背景，不是判据）`,
)
 ...
 */
{
   ...
   * ## 一条已知答案的对照行：尺子自己有没有坏
   *
   * 上面所有判据都建立在一件事上——**「那一卷演了没有」这个观测是准的**。
   * 而这个观测本身会坏：`drain()` 漏了块、`stream` 被截断、认字符串的方式变了，
   * 都会让所有卷一起变成「没演过」，**而那时报表印出来的是一片整齐的零**。
   *
   * 零看着像发现，其实是尺子瞎了。所以放一行**答案已知**的进来：
   * 每一世 `story.begin()` 一跑，正文流里就必然有东西
   * （出生那一章在循环开始之前就演完了）。**这一条要是红，
   * 说明 `drain()` 那条观测路径坏了，上面所有数一个也不能信。**
   *
   * ⚠️ 头一版我想用「出生章的正文」当对照，**那样会恒红**：
   * 出生章入口块是 `heading`（有 `title` 没 `text`），而 `drain()` 只收 `text`。
   * **一条恒红的对照行比没有对照行更坏**——它训练人无视这一段。
   * 所以认的是「这一世的正文流非空」，那正是节令判据依赖的同一件事。
   *
   * 79 2026-09-09 撞过这个形状：它用 `narrative.sceneId` 逐步采样，报出 65 处
   * 「从没演过」，**里面有 `birth-farm`**——出生章每世必演，那个荒唐的零
   * 就是尺子坏了的信号。它是运气好撞上的，**这一行是主动放进来的**。
   */
  if (sawAnyText === 0) {
    failed.push(
      `${worlds} 世里一世也没从 drain() 读到正文——每世开局必有正文，` +
        `这个零说明观测路径坏了，上面所有数都不可信`,
    )
  }
