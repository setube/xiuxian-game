 ...
 * ## ⚠️ 射程的真正分界：指向【闭合】了没有
 *
 * 头一版我以为分界在「静态还是运行时」，而那个说法不成立——
 * `{call:spouse}` 里那个人也是运行时造出来的，这一支照样扫得准。
 *
 * 真正的分界是**指称解析闭没闭合**，而审计有个上下游关系：
 *
 * ```
 * 文本字面
 *   ↓
 ...
 *
 * ```
 * {call:X}                X 是 id，指向【闭合】        → 扫得准
 * 「乳母」二字             可能指三种东西，【未闭合】   → 扫不了
 * { house: { head: 弟 } }  那个「弟」落在谁身上运行时掷 → 连不上
 * ```
 *
 * **占位符走查审计的是已闭合的实体指向；写死的称谓要先经过指称解析，
 * 解析结果闭合之后，才谈得上进同一套存在性证据审计。**
 *
 ...
 * 而这一支只读内容层，看不见。
 *
 * ## 证不出来之后怎么修：两问判依赖，【不判处置】
 *
 * ⚠️ 两问的职责要写窄，否则它会被当成修复方案：
 *
 * > **两问用来判「这句话依不依赖那个人在场」和「删掉它这一节还有没有
 * > 叙事承载」。它们不直接决定最终的修复方式。**
 *
 * 而二问有个弱点，必须跟它写在一起：
 *
 * > **「还有别的话」只能证明【不是空白】，不能证明【原节的核心事实
 * > 仍然成立】。** 一句话如果本身就是这一节唯一的事实承诺，
 * > 那么哪怕整节还有别的文字，它也可能得重写。
 *
 * 所以逐条记账要分四层，不能只写一句「都对上了」：
 *
 * ```
 * 一  规则执行了吗
 * 二  这一条的【原始判定】是什么
 * 三  判定跟【实际处置】一致吗
 * 四  不一致的话，是规则错、处置超出规则，还是记录层级不一致
 * ```
 *
 * `mourning#sour` 那一条属于第四种：按两问是乙档，实际处置是整条分流改写
 * ——**「句子分类」和「整节处置」被我混用了，不能拿处置结果反推分类。**
 *
 * ## 两问本身
 *
 ...
 *
 * ## ⚠️ 而两问判的是【单句】——处置单位有时候比句子大
 *
 * `kindred:mourning` 那次我没按两问走，而回头对的时候才看清为什么：
 *
 * ```
 * sour 那一节   blocks 里有一句不依赖她（「老人家最后那两年，饭是自己烧的」）
 *               按两问：一问是、二问有 → 【乙档】
 * 而我的处置     整条分流走新支 she-went-first → 【甲档】
 * ```
 *
 * 不吻合，而处置是对的：`fond` / `sour` / `cold` 三支是**婆媳关系的三个档**
 * ——她不在了，三档【一个也不成立】。这时候要换的不是某一句，是整条分流。
 *
 * **所以两问之前还有一问：这个人是不是这一整组分支的【分档依据】。**
 * 是的话处置单位是分流，两问不适用；不是的话才逐句问。
 *
 * 而这一条是写完那句「五处全部吻合」之后回头核出来的
 * ——**当时并没有真的一处一处对过**。
 *
 * ## 报数，不判成败
 *
