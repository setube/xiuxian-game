 ...
 * 而用得上它的那一卷窗口当时是十三到十六。也就是说这条路
 * 十有八九只赶得上最后一年——**旗标设上的时候，那扇窗快关了。**
 * 它当时还通，可再往前挪一岁窗口就会当场断掉，
 * 而断掉的样子跟今天一模一样：名单上多两卷，谁也说不清为什么。
 *
 * 2026-09-08 那扇窗挪到了成年段末（`GROWN_UP`，`scripts/ascent.ts` 守着）：
 * 真世里对上线索的 400 世 1 世 → 4–10 世。上面那段留着，它是这条路曾经多窄的记录。
 */
console.log('\n=== 七、可观测路径：真人生里走得到吗 ===\n')
{
  /**
  ...
  if (enteredCrossed === 0) {
    ...
    if (extra.enteredCrossed > 0) {
      ...
      /*
       * ⚠️ **这里从前判红，写的是「这条路是断的，不是稀的」。那句话不成立。**
       *
       * 2026-09-12 顺着漏斗一环环查到底，最后一环是这样（6000 世）：
       *
       *     攒够两条线索      59 世
       *     两条指到同一处     0 世
       *
       * 判红那句话说「断了」，而它断不了：链条每一环都在，
       * `leadsHeard` / `crossed` / `leads-crossed` 三头齐全。
       * 线索池是**带权重**抽的，按权重算「任取两条撞上同一处」只有
       * 5.2%——权重偏向只出现一次的那几条，把它压下来了。
       *
       * ⚠️ **这一段是 2026-09-12 补线索之前的账，留着是为了让改动可查。**
       * 那天把线索池从 10 条加到 13 条（南山从零条到三条、白石桥补了
       * 一条同源的、云台补了一条真的），碰撞率 5.20% → 9.77%。
       * 同一天 2000 世实跑：走进这一卷 4 回，走到门前 1 回。
       * 从「6000 世零人」到这个数，这一节不再是那笔账的记录，
       * 而是它的对照。
       *
       * ⚠️ **59 世期望撞上 3.1 世，掷出零来是 4.5%——低，但不是零。**
       * 这个数不够判「断了」，也不该当噪声打发：
       *
       *     判红   说的是「链条坏了」，而链条没坏——**结论比证据大**
       *     全绿   说的是「没事」，而 4.5% 偏低得值得有人看一眼
       *
       * 所以只报数不判红，**把 59 和 5.2% 一起印出来**，让下一个人
       * 拿着这两个数直接判，不必从头查一遍漏斗。
       *
       * 底下「门前那一卷」那段注释早就写着同一个道理：
       * 「红灯的原因会是抽样而不是内容，那正是这一整套走查最忌讳的一件事。」
       * **同一支门禁里，上面这条犯了它下面刚警告过的错。**
       *
       * 真要守这一层，得先把分母做大——那 5.2% 是内容层的账：
       * 八条线索散在五个去处，而「两条撞上」是这一整条路唯一的门。
       */
      // 主批次的漏斗已经解构在上面了；追加那一批单独加进来
      const enough = funnel.攒够两条 + extra.funnel.攒够两条
      console.log(
        `
  ·  ${LIVES + RETRY} 世里没有一个人走进那一卷。` +
          `攒够两条线索的有 ${enough} 世，而按权重算两条撞上同一处只有 5.2%` +
          `——期望 ${(enough * 0.052).toFixed(1)} 世，实测 0。`,
      )
      console.log(
        '     链条三头齐全（leadsHeard / crossed / leads-crossed），断的不是路，是分母。' +
          '要让这一卷真演得到，得把线索池的去处做集中些——那是内容层的账。',
      )
    }
  }
  /**
