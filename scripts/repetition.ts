...
/**
 * 触及次数少于这个数的条目不参与判定：两次里白说一次也是 50%，那是噪声不是毛病。
 *
 * ## 12 这个数算过了，不是拍的
 *
 * 这一条是**概率性判据**（判的就是一个比率），所以要问它的检出力：
 * 一条知识真实白说率是 p 时，`ENOUGH_HITS` 次触及里能不能超过
 * `TOO_REPETITIVE`（0.35）而被判出来。触及 12 次要超 35% 就得白说 ≥5 次，
 * 二项分布算下来：
 *
 *     真实白说率 p     12 次触及里判出来的把握
 *          35%                42%
 *          45%                70%
 *          55%                89%
 *          65%                97%
 *
 * **恰好在门槛（35%）附近判不准，那是对的**——边缘案例本就不该判死。
 * 真正脏的（一半以上白说）有九成把握抓到，那是这一条要防的东西。
 *
 * 抬到 30 次的话 45% 那档从 70% 升到 86%，代价是入样条目从 36–39 条掉下去
 * （触及满 30 次的知识条目少得多），**判得准的少了，漏掉的多了**。
 * 12 是这两头之间的取舍点。
 *
 * ⚠️ 这个论证依赖 `TOO_REPETITIVE`。那个门槛动了，这张表要重算。
 */
