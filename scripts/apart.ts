...
const HANDED_OVER: Readonly<Record<string, string>> = {
  ...
  /*
   * 东邻添丁那一卷（`life/neighbour.ts`）。
   *
   * 这一支摆的是玩家自己的局，走不到邻居家里去——那一卷的入场压着
   * 「东邻男主人还在、女主人还在而且没过育龄、还没添过这个娃」三条，
   * 而且窗口只开在玩家五到十四岁。
   *
   * `neighbours.ts`（东邻西舍那一支）跑真人生量它，而且比这里量得细：
   * 演到的占比、两支各走到几次（他家本来有没有孩子）、
   * 娃落进哪一户、姓跟那一户合不合、她后来长到多大、有几世先走一步。
   *
   * ⚠️ 尤其是姓那一条：`who` 不写 surname 是「跟本家同姓」，而那一问
   * 从前不带户。东邻是全库头一户姓跟玩家不一样的——把那个参数撤回去，
   * 77 世里 77 世姓错，而这一支和别的门禁一条也不会响。
   */
  'neighbour:east-born#night': 'neighbours.ts',
  /*
