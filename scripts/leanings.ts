...
console.log('\n=== 六、火种寄生在已经发生过的事上 ===\n')
{
  const kinds = { 旗标: 0, 知识: 0, 日录标记: 0, 出身: 0, 关系: 0, 户: 0 }
  /**
  ...
  for (const spark of SPARKS) {
    ...
    for (const condition of spark.requires ?? []) {
      ...
      /*
       * ⚠️ **这一档是补上的，而它缺席的那段时间里这张表一直看着是对的。**
       *
       * `the-house-is-yours` 从前问 `flag: heads-the-house`，2026-09-13 改判
       * `house: { head: 'me' }`（旗只记承户，分家自立的人当着家却没有旗）。
       * 改完那一刻它从「旗标」那一档掉出来，**落进了没有任何一档的空档**——
       * 合计从 43 个火种里少认一次，而这张表一声不响。
       *
       * 这一节想证的是「火种全都寄生在已经发生过的事上」，
       * 而一个数不到的条件种类，看着正像「没有火种认这个」。
       */
      if (condition.house) kinds.户 += 1
    }
  }
