...
const PATHS: readonly Path[] = [
  ...
  },
  {
     ...
     * 受封之国：他自己成了亲王。
     *
     * ## 这一条是这张表里唯一一个「同一个词，两个方向」的验证点
     *
     * 「王爷」在库里出现两次，各在一张表里：
     *
     *     RANK_CALLS   亲王 · 生父 · 礼上 → 王爷    **他跪着叫他爹**
     *     HONORIFICS   亲王 → 王爷                  **一府上下叫他**
     *
     * 上头那条已经由 `demoted` 那条人生验着（宣旨那一节）。这一条验的是
     * 底下那条——而两条并排跑起来才看得出这一层为什么拆成两张表：
     * **同一个爵位，叫的人换了，走的是不同的一张表。**
     *
     * 走一步就够：`open` 那一节落 `identity: '亲王'`，而它标着
     * `manner: '礼上'`（旨意是跪着听完的），于是这一条同时销掉
     * 第五节那笔「标了礼上却没人走到」的账。
     */
    id: 'invested',
    label: '受封之国：一府上下叫他王爷',
    origin: 'court',
    gender: '男',
    steps: [{ scene: 'royal:invest', node: 'open' }],
  },
  {
    /**
     * 中了秀才。
     *
     * **这是这张表里头一条不靠出身、靠自己挣来的敬称。** 上面那六条
     * 全是生下来就带着的（皇子、公主、世子、郡主），或者被一道旨意
     * 夺走的（削爵、削藩）——它们验的是「别人怎么称呼他跟着身份走」，
     * 而身份是别人给的。
     *
     * 这一条不一样：一个农户的孩子考中了，村里人改口叫他相公。
     * 同一层机制，来源完全不同——**而这一层分不出它们的来源，
     * 那正是对的**：称谓只问「你此刻是什么」，不问「你怎么变成的」。
     *
     * 走两步：`roll` 那一节掷榜，`xiucai` 那一节才落 `identity`。
     */
    id: 'xiucai',
    label: '农户的孩子中了秀才：村里人改口',
    origin: 'farm',
    gender: '男',
    steps: [
      { scene: 'exam:again', node: 'roll' },
      { scene: 'exam:again', node: 'xiucai' },
    ],
  },
  {
    /**
     * 没考中，去坐馆。
     *
     * 跟上一条并排放着才看得出这一层的分寸：**「先生」认的是他做的事，
     * 「相公」认的是他的功名。** 一个没考中、靠教几个孩子认字过活的人
     * 照样被叫先生，而他一辈子不会被叫相公。
     *
     * 两条一起才验得了这一格：只有秀才那条的话，「敬称跟着身份走」
     * 这句话会被读成「敬称跟着功名走」——而库里恰好有个反例。
     */
    id: 'tutor',
    label: '没考中，去人家坐馆：先生',
    origin: 'farm',
    gender: '男',
    steps: [
      { scene: 'exam:done', node: 'open' },
      { scene: 'exam:done', node: 'teaching' },
    ],
  },
  {
    id: 'farm',
...
function calledBy(): string[] {
   ...
   * ## 第二条从前问错了一件事
   *
   * 它原先写的是「出生那一卷里没有封号那一节，走完却带上了敬称」——
   * 那句话把「敬称」和「出生封号」画了等号，而库里加了两个自己挣来的敬称
   * （中了秀才的「相公」、坐馆的「先生」）之后，它把两条正常的人生报成了错。
   *
   * **判据要问的那件事没变：敬称得是这一路上某一步给的。**
   * 变的是「哪一步」——从前只有出生那一步，现在中间任何一步都算。
   * 所以现在比的是**头一个记号**（还没走的时候）：
   * 那时没有敬称、走完有了，说明是路上某一步给的，那正是对的。
   *
   * 这一改让判据比从前更严：从前它只管住了出生那一节，
   * 现在它管的是「敬称有没有出处」，而出处可以是任何一节内容。
   */
  for (const one of walked.values()) {
    ...
    /*
     * 生下来没有封号，走完有了敬称——**得能指出是哪一步给的**。
     * `marks` 逐步记着身份，两步之间变了就是那一步给的；
     * 一步也没变而末了有敬称，那就是凭空长出来的。
     */
    if (!one.titledAtBirth && titled.length > 0) {
      const gained = one.marks.some((at, i) => i > 0 && at.identity !== one.marks[i - 1]!.identity)
      if (!gained) {
        wrong.push(
          `${one.label}：走完带着敬称（${titled[0]!.identity}），` +
            '可这一路上没有任何一步改过身份——那不是内容给的',
        )
      }
    }
    /*
     * 生下来没有封号的那几条，头一个记号上也不能有。
     *
     * 头一个记号是**还没走任何一步**的时候，所以它是这一路的起点。
     * 宗室那几条起点上就有封号（出生那一卷给的），那是对的；
     * 农户那几条起点上要是有，那才是凭空长出来的。
     *
     * ⚠️ 我把这一条写反过一次：写成「`titledAtBirth` 而头一个记号没封号就报错」，
     * 六条宗室人生全红——**而它们本来是对的**。`titledAtBirth` 说的是
     * 「出生那一卷里有封号那一节」，不是「走之前就已经封上了」，
     * 那一节得走过才算数。判据问的是起点，前提却拿了终点的话。
     */
    if (!one.titledAtBirth && honorific(atBirth.identity)) {
      wrong.push(`${one.label}：生下来不该有封号，头一个记号上却带着「${atBirth.identity}」`)
    }
  }
