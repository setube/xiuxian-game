 ...
 *
 * ⚠️ 判到哪儿了：**真改过 17 处**（从 git 数出来的，不是凭记忆）
 *
 * ```
 * 83a6f03  childhood 8 处   护送那晚受伤、衙门腰牌被收
 * bdc8f95  royal / schooling / trades 各 1 处
 * 37140a5  illness 2 处     病殁那一节「他没能熬过去」
 * 2272663  dearth 1 处      挑柴那一句
 * d7330ac  trades 3 处      路上遇见那一节
 * ```
 *
 * ⚠️ **候选数不写在这儿**——它每改一处就变，写死了迟早过期，
 * 而一个过期的数比没有数更坏。跑一次就知道，跑法在文件头第一行。
 *
 * 判过而**不改**的（代词指的是别人，或那个人恒定一种性别）：
 *
 * ```
 * trade:guest 六处   「他」指夜里投宿的客人
 * riverman 三处      指摆渡的老头
 * trade:drunk 四处   指楼下喝醉的跑山客
 * routine:prime 四处 问 bond:'子'，性别钉得住
 * child:memory 三处  指路上的人／紧跟 {dam}
 * need:illness 两处  指郎中
 * dearth:price 一处  指田主（编年写着「田主缓了一年」）
 * bearing:await 三处 「是个男孩」「是个女孩」两节各自钉死了性别；
 *                    另一处的「她」指怀孕的妻子
 * census:mismatch 三处 「他」指里长（上一句「里长挨着门问人口」）
 * ```
 *
 * **剩下的没判过。**
 *
 * ⚠️ 别把候选数读成「那么多处缺陷」，也别读成「已经清干净了」。
 * 逐处判的办法只有一个：**读那一句的上文和它那一条 `requires`**
 * ——代词指的是最近提到的那个人，不是那个角色记号。
 *
 * ⚠️ 而同一句里可能有**两个代词指两个人**：`trades` 那句
 * 「他又交代了一句：……站着让他过去」前一个是 `{elder}`、
 * 后一个是路上那个人——**只能改前一个，后一个动了这句话就没有对象了**。
 *
