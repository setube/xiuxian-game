...
#
# ## ⚠️ 读报表前先分「摆局型／统计型」，否则会给统计型白写判据
#
# 一条命令分得开，不用跑刀：`grep -oE "RUNS = [0-9]+" scripts/<名>.ts`
#
#     没有 RUNS   摆局型   目的地写死在内容里 → A 刀不红多半是缺口
#     有 RUNS     统计型   守的是跨内容的结构性不变量，压根不问路径
#
# 统计型的判据长这样：「每一章落了身份的，`marks` 里都写着 identity」
# 「1200 世各身份加起来正好 1200」——**A 刀照不出它们是应该的**。
# 它们在 C 刀底下红也有平凡解释：条件全关死 → 人生被清空 → 样本归零，
# 而这几支自带零样本保护（`whence.ts` 那句「这一条什么也没量」）——
# **那条红是保护在正常工作，不是缺陷。**
#
# 2026-09-12 踩过：我从「A 不红、C 红」这个**行为签名**直接推出「路已经在了，好改」，
# 七支里只有两支是那样。**签名对得上不等于机制对得上。**
#
# ## ⚠️ 一张 N 档的分流表，A 刀最多红 N-1 条——「少红一条」是正常的
#
# `meetsAll` 恒真时 `branches` 取的就是**第一条**，所以排第一那档照样命中；
# 恒假时兜底那条（没有 `requires`）也必然命中。**这两档天然验不出条件层。**
#
# 所以看报表时：一张分流表少红一条不是判据没写好。
# 反过来也真——`childhood` 的 `child:memory` 有十二条 `branches` 按出身分流，
# 而判据只摆 `farm` 断言走到 farm，**农户恰好排第一**，于是恒真时照样走到：
# **一条恰好排第一的分支，能让整张分流表的判据全部失效。**
#
# 还有一种把报表洗干净的写法：判「走过的路径里有没有它」（`walked.includes`）
# 而不是「第一个落点是不是它」。串在同一条路径上的几档会互相兜住，
# A 刀只红得动一条。**判法越宽松，报表越干净。**
#
# ## 为什么是这几刀，不是别的
#
...
# **不红才可疑**。别把这一栏的红当成缺陷去修。
#
# ⚠️ 而这一刀的**写法**也要紧，不能图省事写成 `return false`：
#
#     return false                          无条件的节点也一起关掉 ← 错
#     return (conditions?.length ?? 0) === 0  没有 requires 的照旧放行 ← 对
#
# `meetsAll` 原本对空条件返回 `true`（「没提要求就是谁都行」）。写成 `return false`
# 等于连「本来就不设门槛的地方」也封了，于是**整个库走不动**——红出来一大批，
# 而那批红的解释是平凡的（哪儿也去不了），把真正想找的信号淹掉。
# 2026-09-12 f2 手写这一刀时照直觉写了 `return false`，捞上来的东西里混着这一批。
#
# 规矩：**打断一层，只该打断那一层真正负责的判定**。`meetsAll` 负责的是
# 「这条要求成不成立」，不负责「没有要求时算不算数」——后者不是条件层的事。
case "$WANT" in *C*)
  restore
  patch src/engine/conditions.ts \
    'export function meetsAll(conditions?: readonly Condition[]): boolean {' \
