...
// —— 二、同一场旱灾，不同的人生 ——
/**
 * 世数由「几种走向」那一条定，不是由前面那条链定。
 *
 * ⚠️ **决定它的是撞上旱灾的那一小撮，不是总世数。** 300 世里只有十几世
 * 走到那一族，而判据数的是那十几世里出现过几种走向——
 * **一个十几人的样本，数出来的「种数」在 2 到 5 之间晃**，而门槛是 3。
 *
 * 2026-09-13 一手量的（六颗种子，其中 `138d7vb9rnzw` 那颗在全套里报过红）：
 *
 * ```
 * RUNS   六颗种子各数出几种走向
 *  300   2 · 5 · 5 · 5 · 4 · 5     ← 最低那颗掉到 2，越过门槛
 *  600   5 · 5 · 5 · 5 · 5 · 5
 *  900   5 · 5 · 5 · 5 · 5 · 5
 * ```
 *
 * **真数是 5，600 世就完全稳了**，门槛 3 留着两档余量。
 *
 * ⚠️ 那次报红**不是内容挤掉了旱灾**——同种子开两棵树比过：
 * 加内容之前六颗种子是 `3·5·4·4·5·4`（均值 4.2），之后是 `2·5·5·5·4·5`（均值 4.3），
 * **没有系统性下降**。只是之前那颗恰好压在 3 上，随机流一变就掉到 2。
 * 是阈值画在噪声里，不是世界变差了。
 */
const RUNS = 600
