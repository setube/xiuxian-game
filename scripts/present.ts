...
/**
 * ⚠️ 想给这一族另立一支【静态】门禁的人，先读这一段。
 *
 * 2026-09-14 这一支抓到一条真的：`keeping` 那一卷窗口 40–78 岁，
 * 而正文写死「你又找过写字的先生」——`teacher` 立起来就 50 岁，
 * **玩家四十岁那年他八十开外，多半早殁了**。
 *
 * 修完顺手扫了一遍同类（静态：正文点名 · 无 `alive` 守卫 ·
 * 窗口末尾那人 85 岁以上），扫出 **12 处候选**。
 * 逐条查下去，**一条真的也没有**：
 *
 * ```
 * 嫂子 19 岁出场，窗口末尾 94 岁    → 人是可能活到 94 的，静态判不了
 * 师傅 36 岁出场，窗口末尾 98 岁    → 那一卷【自己问了】alive: false → gone
 * ```
 *
 * ## 静态扫为什么必然全是候选
 *
 * 它取的是**窗口末尾**，那是最坏假设；而真世里那一卷多半早就演完了。
 * 实测 `refuge:master` 真演到时玩家中位 31 岁（窗口是 30–62）。
 *
 * > **窗口末尾不是「真演到那一岁」**，
 * > 而两者的差距正是那 12 处候选的全部来源。
 *
 * ## 所以这一族归这一支，不该另立
 *
 * 这一支跑真世、在人真的殁了之后查正文——**它问的是事实，
 * 不是最坏假设**。静态那一版能做的它都能做，而且不会报出 12 处待查。
 *
 * ⚠️ 查那 12 处时我的探针连着三次印出零样本的「✓」
 * （`stream` 采不到那些正文、`sceneId` 要在 `choose` 之前采）。
 * **「✓ 没问题」和「一条也没采到」印出来一模一样**——
 * 这也是这一族该留在真世走查里的理由。
 */
...
// 「娘家」是一个地方（乳母那件往事：「自己的孩子留在乡下，托给了娘家」）
//   ——2026-09-14 撞到：那笔 recall 挂的是 `nurse`，而回执文本里的「娘」
//   被判据当成了殁掉的母亲。跟「姑娘」「新娘」同一族：一个字的称呼是别的词的零件
const INNOCENT_CONTEXTS: readonly string[] = [
  '姑娘',
  '新娘',
  '娘娘',
  '娘家',
 ...
 */
{
  ...
  if (unique.size > 0) {
    ...
    bad += 1
  } else if (deathsSeen === 0) {
    /*
     * ⚠️ 零样本跟通过**不能共用一个出口**。
     *
     * 这一条要等的是「家里死人，而且死在人生走完之前」。一百世下本该采到
     * 三四百个——可只要摆局改坏、取样点挪错、或者有人把 `RUNS` 砍小，
     * `deathsSeen` 就可能是 0，**而它照样走进那个 ✓ 分支**：
     *
     *     ✓ 0 个人不在了之后，没有谁还在正文或选项里露面。
     *
     * **那句话字面为真，而它什么也没验。**「一个都没采到」跟「采到几百个
     * 全是干净的」印出来只差一个数字，而读的人看的是那个 ✓。
     *
     * 同伴 2026-09-14 一轮里连着三次栽在这上头（`stream` 采不到、`sceneId`
     * 要在 `choose` 之前采、两支都没采到），三次都是自己写的三元判断句
     * `n === 0 ? '✓' : …` 在骗自己。
     *
     * 写成三档才分得开：**零样本 / 通过 / 不通过**。
     */
    console.log('  ⚠️ 一个死者也没采到——这一条这一轮什么也没验（不是通过）。')
    console.log(`     ${RUNS} 世本该采到三四百个。先看摆局和取样点，别看内容。`)
    bad += 1
  } else {
...
/*
 * 四、尺子自检：那三条豁免还认得出它们该认的东西吗。
 *
 * ## 为什么非有这一节不可
 *
 * 2026-09-14 这一支加了三道**放过**（说话人视角、别人对玩家的称呼、
 * 五种死亡说法）。每一道都让它报得更少——**而「报得更少」有两种成因**：
 *
 *     一个【变窄】的判据   候选变少
 *     一个【失灵】的判据   候选也变少
 *
 * **报表上两者一模一样。** 正则里多打一个字、`titleFor` 改了签名、
 * 词表被人顺手清空——都会让这一支安静地更绿。
 *
 * ⚠️ 同伴那一轮正是靠一条自检才没把「滤掉 29 处」当成进步。
 * 而我加完那三道豁免**一条自检也没写**——查了才发现（各只有定义 + 使用）。
 *
 * 所以这一节钉三句话，每一句都是那三道豁免的**分界样本**：
 * 一句必须被放过，一句必须照报。放过了不该放的、或者不放该放的，都红。
 */
{
  console.log('\n=== 四、尺子自检：那三道豁免还认得出分界吗 ===\n')
  const cases: readonly { text: string; exempt: boolean; why: string }[] = [
    { text: '他说，叔，你替我跟我爹讲一句。', exempt: true, why: '说话人视角（我爹＝他的爹）' },
    { text: '爹站在门口。', exempt: false, why: '同一个字，而这句就该红' },
    { text: '嫂子走在前头。', exempt: true, why: '死亡说法（走在前头）' },
    { text: '嫂子留你住了一夜。', exempt: false, why: '同一个人，而这句就该红' },
    /*
     * ⚠️ 这一句是**打断验证逼出来的**。
     *
     * 头一版四句里，唯一含「走」的是「嫂子走在前头」——**而它本来就该放过**。
     * 于是我把词表从「走在前头」弄宽成「走」去打断，**自检一声不响**：
     * 四句的判定一个也没变。
     *
     * **一个只有「该放过」样本的分界，验不出「放得太宽」。**
     * 要抓变宽，得有一句**含那个字而该照报**的：
     */
    { text: '嫂子走到门口就回来了。', exempt: false, why: '含「走」而不是死亡说法——防词表被弄宽' },
  ]
  let held = true
  for (const one of cases) {
    const got = TALKING_ABOUT_DEATH.test(one.text) || SPEAKER_ANCHORED.test(one.text)
    if (got !== one.exempt) {
      console.log(
        `  ✗ 「${one.text}」该${one.exempt ? '放过' : '照报'}（${one.why}），实际${got ? '放过了' : '照报了'}`,
      )
      bad += 1
      held = false
    }
  }
  // isMyOwnTitle 要有 pinia 才问得了，这儿只验它认得出「不是玩家的称谓」那一面
  if (held) console.log(`  ✓ ${cases.length} 句分界样本各归各位——三道豁免没有变宽也没有失灵。`)
}
