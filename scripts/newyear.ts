...
let RUNS = Math.max(SCOUT, Math.ceil((WANTED / scoutRate) * 1.3))
run(RUNS - SCOUT)
 ...
 * ## 为什么补跑，不是抬 SCOUT
 *
 * `scoutRate` 用的是 `when.length`（**已排除之后**的可判数），
 * 所以那个率本身含了排除这一层——**倒推没算错**。错在**方差**：
 * 探路 150 世撞上的排除比例，跟正式批的未必一样。
 *
 * 实测 `SEED=zw3hz41kymb6`：可判 19、排除 9——**排除占了整批的三分之一**，
 * 而探路那 150 世没撞上那么多。
 *
 * 抬 `SCOUT` 是把每一颗种子的成本都翻上去，为的是修少数几颗的偏差；
 * **补跑只在真不够的时候花钱**。三颗种子实测 150 / 179 / 150 世——
 * 够的那两颗一轮也没补。
 *
 * ## ⚠️ 必须封顶，而且封的是世数不是轮数
 *
 * 头一版只封了轮数（三轮），**打断验证时跑超时了**：模拟「率塌了」
 * （四十世之后不再记样本），每一轮按当前实测率算出的世数都在暴涨，
 * 三轮下来是几万世。
 *
 * `RATE_FLOOR` 兜的是「探到 0」，**兜不住「探到极低」**——
 * 率越低算出来的世数越大，而它正是率塌了那一刻的行为。
 *
 * 所以封世数：`CAP` 之内补得动就补，补不动就让底下那条自检说话。
 * **一支会把批次拖到超时的门禁，比一支报红的门禁更坏**——
 * 前者谁也不知道它在干什么。
 *
 * `CAP` 按实测定：150 世约 63 秒（2026-09-09 量的），
 * **450 世约三分钟**——那是这一支在批次里能占的上限。
 * 再多就该先问「为什么要这么多世」，而不是让它跑下去。
 */
const CAP = 450
for (let round = 0; round < 3 && when.length < WANTED && RUNS < CAP; round += 1) {
  const rate = Math.max(when.length / RUNS, RATE_FLOOR)
  const need = Math.min(Math.ceil(((WANTED - when.length) / rate) * 1.3), CAP - RUNS)
  run(need)
  RUNS += need
}
 ...
 *
 * ## 2026-09-09 它红了一次，而那一次它是对的
 *
 * `SEED=zw3hz41kymb6`：可判 19、排除 9——**排除占了整批三分之一**，
 * 而探路那 150 世没撞上那么多。**倒推没算错，是方差**。
 *
 * 修法是**补跑**（见上面 `run(SCOUT)` 那一段），不是抬 `SCOUT`：
 * 抬 `SCOUT` 把每颗种子的成本都翻上去，为的是修少数几颗的偏差。
 *
 * 所以这一条现在守的是**补跑三轮之后还不够**——那不再是方差，
 * 是率本身塌了（那一卷的条件又紧了一层）。
 *
 * 它仍然不是判据（内容没坏），是量具的自检，所以报的话指向世数和探路，不指向内容。
 */
if (when.length < WANTED) {
  console.log(
    `  ✗ 补跑三轮之后仍不够：跑了 ${RUNS} 世只凑到 ${when.length} 个可判样本，门槛 ${WANTED}。` +
      `\n    （另有 ${ambiguous} 回同步演了别的节令被排除——节令越多这一项越大。）` +
      `\n    这不再是方差（补跑就是为了吃掉方差），是那一卷的率本身塌了：` +
      `\n    先看第二条采到几次，再看它的 requires 是不是又紧了一层。`,
  )
  bad += 1
}
