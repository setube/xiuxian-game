 ...
 * **那句话是错的**：出嫁／入赘实测 **0 世**（底下第六条那段印得出来）。
 *
 * ⚠️ **成因我说错过两次，现在这里只写实测得到的。** 我先前两次都断言
 * 「`life/match.ts` 还没接进 `life/index.ts`，那一卷不会触发」——
 * 而 `match` 那一章 `a13ddc8`（2026-09-07 11:10）就进了 `chapters.ts`，
 * 比我头一次写下这句话早八个钟头。我「复查」时 grep 的是 `life/index.ts`，
 * 而那个文件是从 `CHAPTERS` 摊平出来的，**里头根本不会出现 match 字样**
 * ——查错了文件，两次都查错同一个。`match-offer` 的条件（16–40 岁、
 * 没配偶、不在守孝、家境 ≥20、weight 8）并不苛刻，它该演得出来。
 *
 * **所以「0 世」是实测的事实，「为什么是 0」眼下没有结论。** 底下第六条
 * 只报数、不再给成因。查的时候从这儿起步：`match:offer` 演到过几次、
 * 走到 `wed-into` 那两个节点的路上哪一步断的。
 *
 ...
 * **没查到和查过了长得一模一样。**
 *
 * ## 这两条采不到，而成因未知——这一段只报数，不给成因
 *
 * 从 2026-09-07 到 09-08，这里印的是「原因不在这一支：`life/match.ts`
 * 还没接进 `life/index.ts`」。**那句话是我编的，而且从写下那一刻就是错的**：
 * `match` 那一章 `a13ddc8`（09-07 11:10）就进了 `chapters.ts`，比它早八小时。
 * 两次「复查」我 grep 的都是 `life/index.ts`——而那个文件从 `CHAPTERS`
 * 摊平，里头根本不会出现 match 字样。
 *
 * **一支门禁给出的成因会被当结论用**：读到的人不会回去复核，
 * 他会照着那句话去查 `life/index.ts`，然后什么也查不出来。
 * 所以宁可写「不知道」——不知道是可以往下查的，错的成因不能。
 *
 * 判据仍降为报数不判红（第四条的前提确实一次也没成立），但降级的理由
 * 现在写成实测事实：**采到 0 世**。采到之后（`wed > 0`）它立刻恢复成硬判据。
 */
if (wed === 0) {
  console.log(
    `  ⚠ ${RUNS} 世里没有一世嫁出去或入赘，第四条这一批没被验到。` +
      `\n    这是实测事实；**为什么是 0，眼下没有结论**（先前印在这里的那个成因是错的，` +
      `\n    match 那一章早就接进 chapters.ts 了）。要查从这儿起步：` +
      `\n    match:offer 演到过几次、走到 wed-into 那两个节点的路上哪一步断的。`,
  )
}
if (underSomeone === 0) {
  console.log(
    `  ⚠ ${RUNS} 世里没有一世上头有人当家，第一条这一批没被验到（同上，成因未定）。` +
      `\n    采样点是咽气那年——那时爹娘多半已经不在、家早交到自己手上，` +
      `\n    这一条采不到可能是采样点决定的，不是内容没有。`,
  )
}
