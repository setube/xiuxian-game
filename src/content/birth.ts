...
function settleNeighbours(input: {
  ...
  for (const side of NEIGHBOUR_SIDES) {
    ...
    const head = makePerson({
       ...
       * 邻家的老人也有过年轻的时候。
       *
       * ## ⚠️ 从前这一格是空的，而那让「世交」这件事无从谈起
       *
       * 一手量过（300 世）：**爹身上有往事的 281 世，
       * 而东邻户主 0 件**——`rollPast` 只给自家亲属和田主掷过。
       *
       * 于是「你爹和他爹当年一块儿修过河堤」这句话**没有事实可依**：
       * 两家老人共有一件往事的世数是 **0**。
       *
       * ## 掷的是【同一个池子】，那是要紧的
       *
       * 跟自家长辈共用 `ELDER_PAST`，两家的老人才可能撞上同一件事
       * ——而 `soldiered`（被征去修过一年河堤）这种事，
       * 两个同村的男人同一年被征去，本来就是常事。
       *
       * ⚠️ **共有一件往事 ≠ 他们一块儿经历的**：「都逃过荒」是两场荒。
       * 这一格只提供事实基础，**哪几件算「一块儿」是内容层的判断**
       * ——别在这儿替内容层下结论。
       *
       * 掷 1–2 件（自家长辈是 2–3）：邻家的老人玩家更不熟，
       * 知道他两件事已经算多的了。
       */
      history: rollPast(randomBetween(1, 2)),
    })
