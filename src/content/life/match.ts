...
export const matchScenes: SceneLibrary = {
  ...
  'match:offer': {
    ...
    nodes: {
      ...
      elders: {
        id: 'elders',
        onEnter: [{ type: 'recall', id: 'mother', chapter: 'refused-match' }],
        blocks: [
         ...
         * ⚠️ 娘当年那门没成的亲事。
         *
         * `refused-match`（「本来说的是另一门亲事，后来没成」）是立基掷给
         * 长辈的十件往事之一，`known: false`。一手数过（`scripts/past.ts`）：
         * **写入端一处，读取端零**——写在那儿，没有任何一卷翻开过它。
         *
         * 而这一节正是它该出现的地方：**儿子议亲，娘想起自己当年。**
         * 那不是巧合——`refused-match` 的 `atAge` 是 22，
         * 正是这个时代女子议亲的年纪。
         *
         * ## 为什么不加「她身上有这件往事」的条件
         *
         * **条件层问不了往事**（三十三种条件里没有一种问 `history`）。
         * 而 `kin.ts` 那几卷也一样：只问娘在场，`recall` 碰不碰得上看运气
         * （实测 21%，跟「十件里掷 2–3 件」的概率吻合）。
         *
         * ## ⚠️ 而这一节【不另写正文】——`recall` 自己会说
         *
         * 头一版我在这儿加了一条 `seen`（「娘在灶前站了一会儿，说她当年
         * 也说过一门亲」），条件只问娘在场。`seen` 那支当场报 **89.9%**：
         *
         * > 过半的人读到的是同一种组合……这一节对多数人就是同一段，
         * > 所见这一层在这儿没起作用。
         *
         * 判据说得对，而真因是**我重复造了一句 `recall` 已经会说的话**：
         * `recall` 成功时自己印一句「原来 · 本来说的是另一门亲事，后来没成」，
         * 落空时返回 null 什么也不印（`effects.ts` 那一段）。
         *
         * **它天然只在娘真有这件往事时出声（实测 25%），
         * 而我那条 seen 只问「娘在场」，于是九成人都读到。**
         *
         * 所以这儿只留 `recall`，一个字的正文也不加。
         */
        choices: [
          {
            id: 'agree',
