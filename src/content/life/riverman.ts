...
export const rivermanEvents: readonly LifeEvent[] = [
  {
     ...
     * ## 而写成一个数之后，那句话被稀释掉了两次
     *
     *     2026-09-08  55 → 50（用户拍板「书那条太稀」）  当时实测 4% → 15%
     *     2026-09-09  同一个 50                          实测 11.7%
     *
     * 门槛数字一个字没改。掉下去的那三分之一是这两天所有人往库里加内容
     * 加出来的——事件池变大，涨命数的那些选项占比就小了。
     * **每个人都在动这个数，而没有人认为自己在动它**，也没有任何东西会报。
     *
     * ## 它还有第二个毛病：在窗口里是漂的
     *
     * 逐岁实测命数的第九十百分位：十六岁 45、二十岁 44、二十四岁 48、
     * 二十八岁 51。于是「16–28 岁之间够得着 50」**实际上是一条
     * 「熬到二十六岁以后才开门」的规则**——那不是任何人的本意，
     * 而它同样没写在任何地方。
     *
     * ## 所以改成按位次判（用户 2026-09-09 拍板）
     *
     * `前四分之一` 就是那句话本身：**上游加多少内容它都是四分之一，
     * 哪一岁问它都是四分之一。** 分位线由 `engine/cohort.ts` 那张实测表给，
     * 那张表会过期，可**它过期的时候是整张表一起过期，有一个地方可以去改**
     * ——不像从前，一个数散在这儿，而稀释它的人在别的文件里。
     *
     * 实测（1500 世）：`前四分之一` 那条线在这个窗口内是 26% 上下，
     * 比用户 09-08 拍板时看到的 15% 宽一档——**那正是他这次想要的**，
     * 因为他上次要的 15% 已经被稀释成 11.7% 了。
     */
    requires: [{ attribute: { key: 'fortune', among: '前四分之一' } }],
  },
]
