  ...
  'kindred:newyear': {
    ...
    nodes: {
      open: {
         ...
         *
         * ⚠️ **「这个年过完了」那一笔落在卷尾的 `done` 上，不在这儿。**
         *
         * `untilMonth` 已经在目标月时推**零个月**（`(1-1+12)%12 = 0`，
         * `world.ts` 那句注释写着「已经到了就不必再等一年」）。
         * 那是从「演不到」那一头看的，而它开出了另一头：
         * 年节演完日历还停在正月，入场条件（有老屋、侄儿满三岁）照旧成立，
         * 于是**同一年可以反复演**——300 世实测有一世演了 560 回，
         * 而一辈子最多七八十个年。
         *
         * 那 560 回不是「按满回合上限」（`lifelong` 抓的是那一种），
         * 它每回都正常走完，只是把一世的回合全耗在同一个正月里。
         * 报表上什么也看不出来：没有红灯，只是那个人一辈子没干别的。
         *
         * 推过正月要等**正文全演完**：`newyear.ts` 那一支量的是
         * 「`onEnter` 跑完之后是不是正月」，在这儿推会把 84% 的人推到三月，
         * 判据当场报「只有 84.1% 落在正二月」——**我自己引入的红**。
         * 所以那一笔挪到 `done`（这一卷的收尾节点）。
         */
        onEnter: [
      ...
      'debt-mine-open': {
        ...
        /*
         * ⚠️ 这一节从前是**第二个终点**，它绕过了 `done`。
         *
         * 那不要紧，直到「这个年过完了」那一笔落在 `done` 上——
         * 从这儿收尾的人日历不往前走，于是他们那一年可以接着再过一遍。
         *
         * 一卷该只有一个出口。收尾要做的事（推日历、记账、落旗）
         * 都挂在那一个出口上，**多一个终点就是多一条漏网的路**。
         */
        next: 'done',
      },
      done: {
         ...
         * 这个年过完了。
         *
         * ⚠️ 这一笔**必须落在卷尾，不能落在 `open`**。
         *
         * `untilMonth` 已经在目标月时推零个月（`(1-1+12)%12 = 0`）——
         * 那是为「演不到」那一头修的（从前用 `Condition.month` 问
         * 「碰巧在腊月正月」，四分之三的人一辈子读不到）。
         * 而它开出了另一头：年节演完日历还停在正月，入场条件照旧成立，
         * **同一个年可以接着再过一遍**。
         *
         * 落在 `open` 会引出第三头：`newyear.ts` 量的是
         * 「`onEnter` 跑完之后是不是正月」，在开头推就把八成人推到三月，
         * 那一支当场报「只有 84.1% 落在正二月」。
         *
         * 卷尾才是「过完了」该在的地方：正文全演完，日历再往前走一个月。
         */
