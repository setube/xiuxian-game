...
export const festivalScenes: SceneLibrary = {
  ...
  'festival:midautumn': {
    ...
    nodes: {
      ...
      /**
       * 孩子不在这个家里过日子了。排在最前面：一个人到了这个年纪，
       * 桌上最先空的是这个位置。
       *
       * ## ⚠️ 这一节现在是【中转】：不在家 ≠ 今年没回来
       *
       * `CHILD_AWAY` 答的是一个**持续状态**（他此刻在哪儿过日子），
       * 而「今年中秋回没回来」是**当年的一件事**。
       * 2026-09-13 之前这两件事被压成了一个——他一走就再也不回来
       *（实测：他走之后的中秋 67 次，他不在身边 66 次，在身边 0 次）。
       *
       * **而库里的正文早就承认这两层是分开的**：
       *
       * ```
       * kindred.ts:369  「哥在镇上做木匠的，正月里回来过年。
       *                   人在别处，户在老屋」
       * kindred.ts:390  「{call:nephew}从镇上回来过年，穿着铺子里的青布衫」
       * ```
       *
       * ⚠️ **回来过节【不改 `place`】**——那是他在哪儿过日子那一格。
       * 「回来过三天」和「回来定居」在状态上不该长得一模一样。
       * 所以这一层不落任何持续状态，就在这儿现判。
       *
       * ## 年景是【情境制造器】，不是返乡率的修正值
       *
       * 荒年同时有两个方向相反的现实力量：
       ...
       *
       * **两边都成立，所以年景没有稳定的单向因果。**
       * 不写「荒年返乡率 +15%」那种东西——那是把真实世界的双向压力
       * 粗暴压成单向修正。这里让它产生**两种不同的情境**，
       * 而哪一种落下来由那一年的偶然定。
       *
       * ⚠️ 「去了几年之后回得渐少」**也不做**：离家时间本身没有天然产生
       * 「逐年疏远」的因果（头一年可能想家所以回来，也可能新学徒
       * 请不了假；第三年可能站稳了反而回得来）。
       *
       * ## 偶然在这里做什么
       *
       * > 偶然该解决的是「两个现实上都成立的世界，为什么结果不同」，
       * > **不是代替缺失的世界事实**。
       *
       * 所以 `roll` 只在年景那一层分完之后才掷。
       */
      'child-away': {
        id: 'child-away',
        blocks: [],
        branches: [
          /*
           * 荒年：两边的力量都在，而这一年落到哪一边由偶然定。
           * 排在前面——它是这一支里唯一有具体缘由的那种年。
           */
          { requires: [{ region: { harvest: { atMost: 40 } } }], next: 'child-lean-year' },
        ],
        next: 'child-plain-year',
      },
       ...
       * 玩家读到的是哪一种，那一年就是哪一种。
       *
       * ⚠️ 这一掷**每年重掷**（旗每次进来都被 `roll` 覆盖）：
       * 「今年回没回来」是当年的事，不是一次定终身。
       */
      'child-lean-year': {
        id: 'child-lean-year',
        onEnter: [
        ...
        blocks: [],
        branches: [
          { requires: [{ flag: { key: 'child-came-home', equals: '回来了' } }], next: 'child-came-back' },
        ],
        next: 'child-away-said',
      },

      /** 平常年月：没有特别强的现实拉力，多数年他不回来 */
      'child-plain-year': {
        id: 'child-plain-year',
        onEnter: [
          {
       ...
       * 他回来了。
       *
       * ⚠️ **一格状态也不落**：他还是在镇上过日子的人，回来几天不改这件事。
       * 这一节只写你看见的那几天。
       */
      'child-came-back': {
        id: 'child-came-back',
        blocks: [
          { kind: 'narration', text: '孩子今年回来了，到家已经是十四那天晚上。' },
          {
            kind: 'narration',
            text: '他带了半包点心，说是铺子里分的。吃饭的时候话不多，问了两句家里的事。',
          },
          {
            kind: 'narration',
            text: '过了节他就走了。走的那天早上你没起来送。',
            tone: 'faint',
          },
        ],
        next: 'done',
