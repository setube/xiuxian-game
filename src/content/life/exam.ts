  ...
  'exam:first': {
    ...
    nodes: {
      ...
      failed: {
        ...
        blocks: [
          ...
          { kind: 'narration', text: '回来的路上你走得很慢，进门的时候把脸上的表情收拾好了。' },
          {
            kind: 'narration',
            /*
             * ⚠️ 「他哦了一声」从前写死了性别，而 `{elder}` 是角色记号
             * ——一手量过 199 世里它落到娘 115 世、落到姐 14 世。
             * 前半句刚解析成娘，后半句就说「他」。
             *
             * 这一句本来也不需要性别：要紧的是**那一声「哦」**，
             * 不是哦的人是男是女。
             */
            text: '{elder}问了一句，你说没中。哦了一声，就去忙别的了。',
          },
          {
            kind: 'narration',
      ...
      'let-be-gone': {
         ...
         * 全库零处），而正文里在说这件事——这一句就是。
         *
         * ⚠️ **从前这一节只有一段话。** 玩家读到「那些书还是先生留下的」，
         * 而行囊里一本也没有：**这句话说完，世界里什么也没留下**。
         *
         * ## 为什么落点选在这儿，不在丧事那一节
         *
         * `house.ts` 那句「爹留下的东西不多，他用过的那几样，你一样也没舍得扔」
         * 更像这一环的正主，**而它写在 `seen` 里，`seen` 只有
         * `{ requires, text }` 两格，落不下效果**。要落就得先改那一卷的结构，
         * 或者给引擎加一种「按那人的营生现取一件」的效果——两样都不是最小的。
         *
         * 这一节不一样：它**本来就是一个独立节点**，而且只在
         * 「先生殁了」那一支才到得了（上一节 `branches` 问 `teacher alive`）。
         * 东西也是确定的——**书**，不用按营生猜。
         *
         * ## ⚠️ `count: 1` 而它其实是「那些书」
         *
         * 正文说的是一摞，而这儿只落一件。**有意的**：
         * 这一环眼下要证明的是「留下的东西跟捡来的东西分得开」，
         * 不是「他留下几本」。数目要等有人真的去数它的时候再说。
         */
        onEnter: [
          {
            type: 'item',
