    ...
    nodes: {
      open: {
        ...
        onEnter: [
           ...
           * ⚠️ 娘当年替人担过的那笔债。
           *
           * `old-debt`（「替人担过一笔债，还了三年」）是立基掷给长辈的
           * 十件往事之一。`scripts/past.ts` 数出来：**写入端一处，读取端零。**
           *
           * 而这一节正是它该出声的地方：**家里头一回借钱，
           * 而她当年替人担过债、还了三年。** 上一句写的正是她追问
           * （「{dam}问了一句，他没答」）——她问得比旁人急，是有来由的。
           *
           * ⚠️ 挂在【娘】身上而不是爹：这一卷里爹是借钱的人，娘是追问的人。
           * `old-debt` 爹娘都可能掷到，而这一刻该出声的是她。
           *
           * 不另写正文——`recall` 成功时自己印一句，落空时什么也不印
           * （议亲那一卷刚踩过：多写一句正文，`seen` 报 89.9%）。
           */
          { type: 'recall', id: 'mother', chapter: 'old-debt' },
        ],
        blocks: [
...
export const hardshipEvents: readonly LifeEvent[] = [
  {
     ...
     * ## 窗口为什么从五岁开头，而不是七岁
     *
     * 启蒙那一卷的头一句注释写着：「那年家里有没有余钱，
     * 是前面七年一点一点攒出来或者赔进去的。」——**那句话从前是假的。**
     * 这条链最早也只到七岁，跟启蒙同岁争年表，于是七岁那天量到的家底
     * 永远还是出生那一刻掷出来的那个数，前面七年一分钱也没赔过。
     *
     * 后果不在这一卷，在两卷之外：农户起手 26–38，而读不上书的门槛
     * 正是 26。两个数恰好挨着，中间没有一年可以往下掉，
     * 于是「读不上书」那一节和它底下的四节（下地、窗外偷听、
     * 一年之后、退学）三百世一个人也走不到，识字率量出来是 100%。
     * **落幕那一句「认得字在你这一辈人里不算常见」因此是句假话。**
     *
     * 五岁不是拍的：这一卷是旁观视角——父亲每天傍晚去地里站一会儿，
     * 站着不说话。**这是一个五岁孩子记得住的画面**，再往前推
     * 就得写他记不住的事了。
     */
    id: 'debt-drought',
    window: { from: 5, to: 10 },
