...
export const CHAPTERS: readonly Chapter[] = [
   ...
   */
  {
     ...
     *
     * ⚠️ **这一处红过两次，第二次是我自己的两笔打架。**
     * `4558abf` 把章界定到 8–40，而 `bc85b1f`（修 `present` 那个 bug 那一笔）
     * 把事件窗口抬到了 45——**探针打印里有一次「43 岁前提齐备」，
     * 我顺手放宽了窗口，没回头看章界**。
     *
     * 改窗口和改章界是两个文件里的两个数，而**它们必须一起动**。
     * `verify` 第七道就是为此存在的：它是唯一会说「这两个数对不上」的地方。
     */
    age: [8, 45],
    purpose: [
   ...
   */
  {
    ...
    purpose: [
      ...
      'person.doing 那一格等的就是这件事——一个孩子真的开始做活，该由内容填上',
      '选项改变过程中的事实，不指定结果：说「家里离不开人」不会把他锁在家里',
    ],
    marks: ['person', 'flag', 'chronicle', 'household'],
  },
   ...
   */
  {
    ...
    purpose: [
      ...
      '「别出去乱说」是门内的规矩：跟自家人说留在家里，跟邻家说有一天他不再理你——后果落在关系上，凡间不知道',
      '他不老——凡人看得见的第一样超凡的东西：爹背驼了、自己的手粗了，他还是那个样子；知道山上的读成跟山上有关，不知道的读成山里人硬朗',
      '造册那年里长来药庐，他报了四十五——官府知道的修士就是册子上那一行；发觉过他不老的知道那不是真的，而谁也不会去说',
    ],
