      ...
      open: {
        ...
        seen: [
          ...
          },
          {
             ...
             * 而不是 `branches`：**走到这一卷的人都到这一节**，
             * 而这几句只给察觉得到的那些人。
             *
             * ## ⚠️ 但守这几句的不是 `seen.ts`，是 `scripts/ageing.ts` 第六问
             *
             * 头一版这儿写的是「这一节人人都到」——**那句话会让下一个
             * 读它的人以为 `seen.ts` 在守这四句**（22 指出的）。它不守，
             * 而且守不了。`scripts/mountain.ts` 自己印的数：
             *
             *     随机 300 世    发觉他不老 0    ← 一个人也读不到
             *     有心人 300 世  发觉他不老 44   ← 只有摆局的有心人到得了
             *
             * 真世里走到这一卷是千分之一量级（800 世 1 次实测），
             * 而 `seen.ts` 对样本不足的节点打的是「不足六十世判不了」——
             * **它对这几句永远是沉默的，而沉默跟绿长得一模一样。**
             *
             * 这正是「稀卷里的分句归摆局验」那一条：`ageing.ts` 第六问
             * 摆局验这三句随认识年头递进，`mountain.ts` 第六问验这一节
             * 本身走不走得到。**`seen` 那一层在这儿只是个容器，不是判据。**
             *
          ...
          /*
           * ⚠️ **「认识四十年」那一句从这儿删掉了，因为它在这一卷里恒假。**
           *
           * 玩家十三岁头一回进药庐，认识四十年就是五十三岁——
           * 而这一卷的窗口是 30–`PRIME_UP`（49）。**那一句一个人也够不着。**
           *
           *     认识 20 年 → 33 岁   窗口内 ✓
           *     认识 30 年 → 43 岁   窗口内 ✓
           *     认识 40 年 → 53 岁   ✗ 窗口 49 就关了
           *
           * 是 `seen.ts` 印的 `0.0%` 露出来的（22 提醒我跑那一支）。
           * 而 `ageing.ts` 第六问**没抓到它**——那一问摆局跑，
           * 不受年表窗口限制，在它眼里五十三岁读得到。
           *
           * **摆局验得了「这条路走不走得通」，验不了「这一卷开到几岁」。**
           *
           * 那一句搬去了 `still.ts`（窗口 50 起），在那儿它够得着。
           */
        ],
