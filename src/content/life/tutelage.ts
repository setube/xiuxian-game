 ...
 * 那五句他背不背得下来、明不明白、转不转得动，是另外三件事，
 * 在 `rites.ts` 那四道关口上——而**他不知道自己卡在哪一道**。
 */
export const tutelageScenes: SceneLibrary = {
   ...
   *
   * 四道关口，一次最多过一道，**而玩家不知道自己卡在哪一道**：
   * 背不全的人和背得滚瓜烂熟却一丝气感也没有的人，
   * 看到的都是同一件事——天亮了，该下地了。
   *
   * 中间那一道尤其狠：`明白` 这一关过与不过，正文里**写的是同一段话**。
...
export const tutelageEvents: readonly LifeEvent[] = [
  ...
  },
  {
     ...
     * 练。
     *
     * `repeatable` 是这一条的要害——**四道关口一次最多过一道**，
     * 而转不动那一道根本过不去。一辈子只能练一次的话，
     * 「练了三年也没有动静」这种人生就压根写不出来。
     */
    id: 'tutor-alone',
    repeatable: true,
