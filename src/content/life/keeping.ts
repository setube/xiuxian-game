 ...
 * **全库唯一的使用者是「熬夜伤身，减两年」**（`routine.ts:1046`）。
 ...
 * ## 那个量是量出来的，不是拍的
 *
 * 2026-09-10 实测（`rollSpan` 各掷两万次，`GATE` 之外的临时探针）：
 *
 *     身子骨 40   天年 p25 57  p50 65  p75 73  p90 77
 *     身子骨 50   天年 p25 59  p50 67  p75 75  p90 79
 *     身子骨 60   天年 p25 61  p50 69  p75 77  p90 81
 *
 ...
 *
 * 给 `+8`：p50 的人从 67 到 75。**那不是长生，那是「老得慢了一点」**，
 * 而这一片要写的正是这个。
 *
 * ## 三条不做
 *
    ...
    nodes: {
      ...
      held: {
        ...
        onEnter: [
          ...
          /*
           * 八年。量出来的：凡人天年 p50 = 67（`rollSpan` 两万次实测），
           * 而玩家不掷老病（`story.ts:237` 只问 `isSpent`），
           * 所以这八年是实打实的八年——p50 的人从 67 到 75。
           *
         ...
         */
        branches: [{ requires: [{ flag: { key: 'first-attempt', equals: 'hurt' } }], next: 'sign-sweat' }],
        next: 'sign-winter',
      },
