...
export const DOINGS: readonly Doing[] = [
  ...
  },
  {
     ...
     * 去镇上这一趟，长辈排在最前。
     *
     * 跟上山正相反：**镇上是大人办事的地方**，一个孩子多半是被带去的
     * ——赶集、抓药、送东西。没有大人带才轮到跟哥去，
     * 再没有才是自己走那半天。
     */
    along: ['长辈', '手足', '同伴'],
  ...
  },
  {
     ...
     * 上山这一趟，谁跟着去。
     *
     * 次序是亲疏：有哥就是跟哥去，没有才轮到邻家的孩子。**长辈不列**——
     * 大人上山是去做事（砍柴、采药），那不是「陪孩子走走」，是另一件事。
     */
    along: ['手足', '同伴'],
  },
  {
...
export const BEATS: readonly Beat[] = [
  ...
  },
  {
     ...
     *
     * 冬闲不是「什么也不做」——修农具、搓草绳、编筐，农家冬天
     * 手上照样有活，只是活换了一种。
     *
     * ## 出处：合理化，不是查来的
     *
     * `scripts/attest.ts` 第二道抓到过这一条（原文写的是「明代农家冬天」）。
     * **抓得对**：那句话点了朝代，就是在做一个史实主张，
     * 而我手上没有任何农书或会典条文支持它。
     *
     * 它软在两处：一、「修农具、搓草绳、编筐」是我按常理列的，
     * 不是照某一部农书的记载；二、**南北差得很远**——
     * 北方冬天地真冻着，江南可以种冬麦、可以修水利，
     * 这一条按北方写，而这个世界并没有定死在北方。
     *
     * 所以朝代名去掉了。**不声称是史实，就不必有出处**——
     * 这是 12 给的两条路里的第二条，我选它是因为
     * 这句话本来也不需要靠明代来成立：地冻上了没活干，
     * 是农事本身的道理，不是某一朝的制度。
     */
    doing: 'work',
    tags: ['替家里干活', '冬闲'],
  ...
  },
  {
     ...
     * ⚠️ 这一条**必须问「是不是一个人去的」**。
     *
     * 「山路上没有人」这句话在有伴的那一趟里是错的——**身边就有一个人**。
     * 而这一条的权重是 40，是上山那几条里最厚的一档，
     * 于是从前多数「跟哥上山」的日子读到的是「山路上没有人」。
     *
     * 这正是 23.md 那句「同行者不是陪同 NPC 标签」的反面：
     * 不问他在不在，他就真的不在。
     */
    requires: [{ along: ['独自'] }],
  ...
  },
  {
     ...
     * 同一段山路，两个人走。
     *
     * **这一条跟上面那条是同一档（无事），权重也一样厚**——
     * 有伴不是奖励，只是那一天的另一种样子。
     *
     * 最后那句是这一条的重心：他们没说什么话。**一起待着不等于交谈**，
     * 而写成「你们聊了一路」就成了填充物。
     */
    doing: 'hill',
    tags: ['山那边'],
    tier: '无事',
    weight: 40,
    requires: [{ along: ['手足', '同伴'] }],
    text: [
      '两个人走得比一个人快，到山顶还早。',
      '在能看见村子的地方坐下，各自啃各自带的干粮。',
      '一路上没说几句话，可谁也没提要回去。',
    ],
  },
  {
     ...
     * 有伴才可能出的事：走散。
     *
     * 一个人上山不会走散。而这一条是「处境」不是「大事」——
     * **他找着了，只是回来晚了，挨了一顿说**。
     *
     * 23.md 那句「危险和救助都必须来自环境，而不是为了给玩家制造剧情」——
     * 所以这里没有熊、没有见义勇为，只有两个孩子在林子里绕了半天。
     */
    doing: 'hill',
    tags: ['山那边'],
    tier: '处境',
    ...
    text: [
      '在林子里走岔了。你喊了几声，没人应。',
      '你顺着水声往下走，走到溪边才看见他也在那儿。',
      '两个人都没说刚才害怕。回到村口天已经擦黑，{elder}在门口站着。',
    ],
    effects: [{ type: 'attribute', key: 'will', delta: 2 }],
  },
  {
    doing: 'hill',
    tags: ['山那边'],
