...
export const DOINGS: readonly Doing[] = [
  ...
  },
  {
    ...
    label: '去镇上',
    slots: ['上午', '下午'],
    // 村里镇上的人才「去镇上」；住城里的是「上街」，宫和王府的孩子出不了那道门。
    //
    // **这一条是第一批内容的事实，不是社会规则。** 「出不了门」不是 `manor` 这种日子
    // 的固有属性——世子成年了、获准出府、跟着长辈出行、偷偷溜出去、被送往别处，
    // 都是出得了的。将来该由「住处 + 身份 + 年龄 + 陪同者 + 许可 + 具体事件」一起决定
    // 能不能去；眼下那几格还没有第一个使用者，先只问住处。别把它写成
    // `if (living === 'manor') cannotLeave`（用户 2026-09-06 划的线）
...
export const BEATS: readonly Beat[] = [
  ...
  // ============================================================
  {
    ...
    weight: 42,
    /*
     * 割草是春夏的活。**这一条是 `season` 那一格的第一个使用者**——
     * 从前它一年到头都可能落下来，于是腊月里的孩子「一上午割了半晌草」，
     * 而那时候地里连草都没有。
     *
     * 加 `season` 之前这句话改不了：全库没有一处内容读得到月份，
     * `world.time.month` 只被用来推进时间。
     */
    requires: [{ living: { is: 'farm' } }, { season: { in: ['春', '夏'] } }],
  ...
  },
  {
     ...
     * 秋收。
     *
     * 跟上面那条是同一件事的另一季：地还是那块地，活不是那个活。
     * 从前这两句合在一句里说不出来——「下地」是个笼统的词，
     * 而**割草和收割在一个农家孩子身上是两种累法**。
     */
    doing: 'work',
    tags: ['替家里干活', '替家里下地', '秋收'],
    tier: '无事',
    weight: 40,
    requires: [{ living: { is: 'farm' } }, { season: { is: '秋' } }],
    text: [
      '割了一天稻子，腰直不起来。',
      '大人在前头，你在后头捡穗子。',
      '傍晚回去的时候，车上的谷子压得辙很深。',
  ...
  },
  {
     ...
     * 冬闲。
     *
     * **地里没活了，可日子还得过。** 这一条补的是从前的一个洞：
     * 冬天照样掷得到「替家里下地」，而那时候地是冻着的。
     *
     * 冬闲不是「什么也不做」——修农具、搓草绳、编筐，明代农家冬天
     * 手上照样有活，只是活换了一种。
     */
    doing: 'work',
    tags: ['替家里干活', '冬闲'],
    tier: '无事',
    weight: 36,
    requires: [{ living: { is: 'farm' } }, { season: { is: '冬' } }],
    text: [
      '地冻上了，没什么可下的。',
      '{elder}在檐下修那把锄头，你蹲着看，手冻得通红。',
      '一整个上午就修好了一把。',
    ],
  },
  {
    ...
    tier: '无事',
    weight: 30,
    // 旱年也分时候：冬天地本来就是干的，「锄头下去土块碎成面」那句话
    // 得在还能下地的季节里说才有分量
    requires: [{ living: { is: 'farm' } }, { season: { in: ['春', '夏', '秋'] } }],
    when: { rain: { atMost: 34 } },
