 ...
 *
 * **不是穷人的固定职业，是当前的生计状态**（用户 2026-09-06 划的线）：灾年短期做工的
 * 殷实农户、暂时帮工的学徒、刚破产的商人、主动攒钱的年轻人，过的都是这种日子。
 */
const HIRED: Living = {
  id: 'hired',
 ...
 * 山上。
 *
 * ## 这一档是 29.md「上山」那一卷逼出来的（2026-09-09）
 *
 * 他被叫上山，在半山腰一片旧屋子里挑水烧灶。而在这一档之前，
 * **他的 `living` 还是 `farm`**——于是日常照旧给他「帮家里干活」
 * （`day.ts:67/142`）、「出门做工，挣几个钱回来」（`routine.ts:403`），
 * **而山上没有「家里」，也没有「出门做工」这回事。**
 *
 * 那三处是排除法（`notIn: ['palace', 'manor']`），**新档天然读得到**
 * ——所以加这一档的同时要把它加进那三处的排除名单。
 * 白名单那二十处（`living: 'farm'` 之类）天然读不到，不用动。
 *
 * ## 为什么不用 `temple`
 *
 * 22 的判断，我认：`temple` 的 `summary` 是「寺里。天不亮起，天黑了睡，一日两餐」
 * ——用它他就成了寺里的人，而 `upbringing.ts` 那张按 `living` 归词的表
 * 也会把寺里的词归给他。**`living` 问的是「他过什么日子」，不是「他在哪儿」。**
 *
 * ## `summary` 写的是凡人视角
 *
 * 「没人告诉你为什么」——他不知道这地方是什么，也没有人打算告诉他。
 * 这跟 `mountain.ts` 那一册的立场是一条：**凡间能得到的答案永远只有半句。**
 */
const UP_THERE: Living = {
  id: 'up-there',
  chore: { holds: '一只空水桶', putsAway: '把桶挂回灶房檐下' },
  summary: '半山腰。挑水、烧灶、听人使唤，没人告诉你为什么',
}
