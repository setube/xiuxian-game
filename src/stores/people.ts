...
export const usePeopleStore = defineStore(
  ...
  () => {
     ...
     * 他看着多大。
     *
     * **绝大多数人这个数就是 `ageOf`**——样子跟岁数一起走，那是常态。
     * 只有 `seemsAge` 那一格写过的人不一样：他的样子停在那一年，
     * 此后岁数照涨，样子不动。
     *
     * 跟 `ageOf` 同一条纪律：**算出来的，不存**。存一个「看着多大」
     * 就是又一个「28 岁。还在襁褓里」——时间往前走而那个字段不动。
     *
     * ⚠️ **这一格答的是外表，不是寿数。**「他还能活多久」是
     * `span - age`，跟这个数没关系：一个样子停住的人可能明天就没了
     * （伤重、道基毁了），一个头发全白的人可能还有一百年好活（秦守拙）。
     */
    function seemsOf(id: string): number {
      const person = roster.value[id]
      return person?.seemsAge ?? ageOf(id)
    }
...
export function makePerson(input: {
   ...
   *
   * ## 这一格是被库里四个人逼出来的，不是先造的阶梯
   *
   * 用户 2026-09-10 把寿命拍成三层：延寿（上限增加、明显衰老）／
   * 不老（衰老速度变了、外表停住）／长生（不受凡人尺度支配）。
   * 而 `span` 那一格**只落得了头一层**（`content/cultivators.ts` 那段
   * 注释早就写着：「引擎眼下看不见不老，它只在正文里」）。
   *
   * 于是库里四个修士在引擎眼里一模一样，而正文里是两种人：
   *
   *     秦守拙（庙里）    108 岁   「他是个老人。头发全白了」   ← 延寿
   *     药庐那位          84 岁    「四十岁再进去还是那个样子」 ← 不老
   *     山道上那位        123 岁   「他看着不到三十」           ← 不老
   *     下山取药那个      —        「看着比你还小几岁」         ← 不老
   *
   * **一格分不出这两种人，而他们天天在玩家眼前走来走去。**
   *
   ...
   * 哪天真有一个「老得比常人慢一半」的人写出来，那时候再加那一格。
   *
   * ⚠️ **这一格只说外表，不说寿数。** 一个人可以停住样子却快死了
   * （道基毁了、伤重），也可以明显衰老却还有一百年好活（秦守拙）。
   * 「还能活多久」是 `span - age`，跟这一格是两件事——
   * 用户 2026-09-10 定的：**长生尺度描述的是「未来还能活多久」，
   * 不是「活到了多少岁」，更不是「看着多年轻」。**
   */
  seemsAge?: number
  place: string
  ...
  return {
    ...
    // 样子停在几岁。凡人不传——他们看着就是实际的岁数
    ...(input.seemsAge === undefined ? {} : { seemsAge: input.seemsAge }),
    place: input.place,
    fate: '在',
