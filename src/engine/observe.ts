...
export interface Remark {
   ...
   * 真值 62 他看成 38 说了「寻常」——那才叫说岔了。
   *
   * 分档本来就是一层粗筛：判断力不足的人多半只是在同一档里晃，
   * 说出来的还是那句话。**偏差要跨过档位的坎，才变成一句错话。**
   *
   * ## 这个字段绝不进 store
   *
   * 它只是 `observe()` 的返回值。`effects.ts` 落笔时只取
   * aspect / source / text / doubt——**玩家若能知道哪句是错的，
   * 那就不叫错了**。它存在只为两件事：门禁验得着，
   * 以及日后「多年以后重新理解」那一层有个东西可以对照。
   */
  astray: boolean
  /**
   * 他量到的那个数。
   *
   * 跟 `astray` 一样**只在引擎里活着，不进 store，也不给玩家**。
   * 它有一个用处：**他接下来怎么待你，是按这个数，不是按真值。**
   *
   * 一个看不见资质的炼气修士量到你「悟性一般」，就随口应付两句走了——
   * 而你可能是万里挑一的胚子。他没有说谎，也不是瞧不起你，
   * 他只是量了另一样东西，而那一样恰好不出众。
   */
  held: number
}
...
export function observe(observer: Observer): Remark[] {
  ...
  return observer.readings.map((reading) => {
    ...
    return {
      ...
      // 说出口的那句，跟真值本该得到的那句，是不是同一句
      astray: words !== phraseFor(reading.phrasing, truth),
      held: seen,
    }
  })
}
