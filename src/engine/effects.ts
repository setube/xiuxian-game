...
export function applyEffects(effects?: readonly Effect[]): NarrativeBlock[] {
   ...
   *
   * **但多条 `time` 同批不再是无所谓先后的。** 从前这里写着「推进的是天数，
   * 加法不挑顺序」，那句话在只有年月日三格的时候成立；`untilMonth` 是**目标**
   * 不是增量（走到几月为止），它跟增量之间不满足交换律——
   * 「推到正月，再加两天」和「加两天，再推到正月」在月底那两天会分道。
   * 上下文相是**按数组原序**结算的（底下那个 for 循环），所以内容里写的顺序
   * 就是生效的顺序：节令那几卷一律先 `untilMonth` 后 `days`。
   */
  /**
